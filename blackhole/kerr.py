...
N = PX.size
# local direction in the observer's orthonormal (r, theta, phi) frame: forward is -r, up is -theta, right is -phi
n_r = -np.ones(N); n_h = -tany * PY; n_p = -tanx * PX
k = 1.0 / np.sqrt(n_r * n_r + n_h * n_h + n_p * n_p)
n_r *= k; n_h *= k; n_p *= k
# coordinate momentum p^mu = e_(t)^mu + n_a e_(a)^mu, then lowered
...
p_h = g_hh * pu_h
E = -p_t
p_t = p_t / E; p_p = p_p / E; p_r = p_r / E; p_h = p_h / E          # energy 1 at infinity

r = np.full(N, R_OBS); th = np.full(N, TH_OBS); ph = np.zeros(N)
state = np.zeros(N, np.int8)                                         # 0 flying, 1 hit, 2 fell, 3 escaped
...
while active.size and step < MAX_STEPS:
    ...
    dl = np.clip(0.04 * (ra - r_h), 0.004, 0.5) * np.clip(np.sin(ta) / 0.25, 0.04, 1.0)   # small steps near the poles
    ...
    zc = (np.cos(ta) * np.cos(tn) < 0)                                 # crossed the equatorial plane
    ...
    step += 1
    if step % 1000 == 0:
        print(f"  step {step}: {active.size} flying, {int((state == 1).sum())} hits, {int((state == 2).sum())} fell, {int((state == 3).sum())} escaped ({time.time() - t0:.0f} s)", flush=True)
state[active] = 2
...
def stars(dirs):
    ...
    return (hsh < 0xFFFFFF * 0.004).astype(np.float64) * (0.3 + 0.7 * ((hsh >> 8) & 0xFF) / 255.0)
...
def band_noise(nr, nph, k_lo, k_hi):
    ...
    kk = np.hypot(fr * nr / 8.0, fp * nph / 40.0)
...
for n in range(NF):
    ...
    col = ramp(temp_scale * 1.15) * (I / (1.0 + I))[:, None] * 1.5 + sky_rgb
