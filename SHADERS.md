...
5x5 texels of their level, and all of it was tuned on a 1280x720 ladder and
1080p footage. Run unchanged on a 4K frame, the reach halves as a fraction
of the screen and the coarse level sees half the context. This file is the
variational build with every pyramid divisor doubled (1/32, 1/16, 1/8, 1/4
of the frame instead of 1/16 to 1/2), so each level of a 4K frame has the
texel count the shader was tuned on; everything expressed in a level's own
texels is unchanged by construction, and the warp converts the finest
level to the frame with 4.0 instead of 2.0. `tests/scale_shader.py` makes
it, checks that every hand-off between levels was rewritten, and refuses a
shader with a pixel constant it does not know. Its caches were sized for
4K at the original divisors, so it is safe to 8K.

Measured 2026-09-05, RX 6600. The synthetic ladder at twice its size (the
same scenes, lanczos 2x on the shader's input and on the truth), the shipped
file against this one, with the shipped file at native size for reference:

| case | shipped, 1280x720 | shipped at 2560x1440 | this file at 2560x1440 |
|---|---|---|---|
| L1_trans_8px | 54.24 | 53.69 | 53.64 |
| L2_trans_16px | 50.17 | 42.89 | 50.23 |
| L3_trans_23px | 41.60 | 37.14 | 41.59 |
| L4_trans_40px | 33.96 | 32.12 | 34.15 |
| L7_textured_large | 23.37 | 21.54 | 23.44 |
| L8_diagonal | 45.76 | 41.47 | 45.43 |
| O5_osc_textured | 40.33 | 42.47 | 40.54 |
| A5_accel_tex_a067 | 43.10 | 54.94 | 42.57 |
| R3_rot_tex | 33.95 | 39.67 | 33.24 |

This file at twice the size reproduces the shipped file at native size,
case for case, which is what it is for; the shipped file at twice the size
loses 4 to 7 dB wherever the motion exceeds its halved reach. Where the
motion is well inside reach (A5, R3) the shipped file at 2x scores far
higher, and that is the test's own smoothness: a lanczos upscale has no
energy above half its Nyquist, which makes sub-texel matching at the fine
levels unusually clean. Real footage, which carries detail to its own
Nyquist, does not show it -- decimate-and-reconstruct on the reference clip
upscaled to 3840x1616, five segments:

| | PSNR mean | SSIM mean |
|---|---|---|
| shipped file at 3840x1616 | 34.77 | 0.9772 |
| this file at 3840x1616 | **36.29** | **0.9808** |

up on every segment, and about where the shipped file sits at its native
1080p (36.31 / 0.9750). A third design, the three coarse levels doubled and
the finest kept at half resolution for the warp's benefit, measured no
better on the textured cases (A5 42.59, R3 33.29), a little worse on the
reach ladder (L2 48.92, L8 44.43) and the same on footage (36.34 / 0.9807),
so it is not shipped; `scale_shader.py` can still make it.

Time from a file at 3840x2160, 61 frames 24 -> 60, median of three
interleaved: stock mixer 1.39 s, shipped file 3.34 s (17.9 fps), this file
2.98 s (20.1 fps). The search work runs on a quarter of the texels.

What it does not do: the two full-resolution edge masks keep their
one-pixel neighbourhood. The human-reading tail reports the frame's own
pixels at twice its usual full scale (a machine reader decodes at 64 px for
velocity, 4 for acceleration and jerk), so a painted field is comparable
with the original's. The tool also takes the generated tri/quad/quint (the
extra slots, the fused hand-offs, the full-resolution flow level and the
pixel constants of the final pass); the scaled quad's 4K disc is discussed
in NFRAME-LIMITS.md section 9, and no field shader is shipped at 4K until it
has a ladder at 4K.

**On a real 4K film (2026-09-06).** A 3840x2076 ten-bit HEVC film at 24 fps
from the owner's library, five three-second segments screened for full
per-frame motion, decimate-and-reconstruct at the native size, RX 6600:

| segment (s) | hold | linear | unscaled | this file | hold | linear | unscaled | this file |
|---|---|---|---|---|---|---|---|---|
| 600 | 34.94 | 37.45 | 38.67 | **38.87** | 0.9677 | 0.9725 | 0.9751 | **0.9752** |
| 2400 | 32.82 | 35.83 | 37.90 | **38.39** | 0.9495 | 0.9617 | 0.9693 | **0.9701** |
| 3000 | 26.25 | 28.25 | 29.46 | **30.64** | 0.9203 | 0.9313 | 0.9447 | **0.9565** |
| 4200 | 26.04 | 28.18 | 31.25 | **34.72** | 0.9182 | 0.9311 | 0.9545 | **0.9643** |
| 4800 | 30.07 | 32.36 | 34.34 | **35.46** | 0.9441 | 0.9515 | 0.9630 | **0.9666** |
| mean | 30.02 | 32.41 | 34.32 | **35.62** | 0.9400 | 0.9496 | 0.9613 | **0.9665** |

PSNR on the left, SSIM on the right. This file is above the unscaled shipped file on every segment on both metrics,
and the ten-bit source, the film's own aspect (2076 rows, not a multiple
of 32) and the seek into a long HEVC stream gave the pipeline nothing to
stumble on: no hook skips beyond the boundary ones, no compile or parse
alarms, the retained frames back near-perfect. Time from a file at the
film's size, 158 output frames 24 -> 60, median of three interleaved:
stock mixer 5.742 s, this file 8.759 s (18.0 fps), the unscaled shipped file
9.897 s (16.0 fps). Temporal steadiness on six seconds of the film holding two cuts (tests/jitter.sh): the phase profile over the five-frame cycle with the cut frames excluded is 1.26x for this file, 1.28x for the unscaled file and 1.36x for the stock blend (the tool's even reference is 1.09x; with the cuts left in the raw figures are 1.98x, 2.04x and 1.54x, which is what two cuts do to a five-phase mean), and both shaders reproduce a cut in one output frame where the blend smears it over three. Under synthetic grain added to the same film (ffmpeg's noise filter at strengths 6 and 12, independent per frame, the grained frames themselves the truth) the order holds at every strength: this file 37.39, 35.89 and 33.39 dB at grain 0, 6 and 12 against the unscaled file's 37.08, 35.64 and 33.24 and the stock blend's 35.48, 34.18 and 31.93 (SSIM 0.9658, 0.9157, 0.7911 against 0.9656, 0.9152, 0.7907 and 0.9604, 0.9048, 0.7691), the margin over the blend narrowing from 1.9 to 1.5 dB and never inverting; on the heaviest grain the phase profile is 1.36x for this file and 1.40x for the unscaled one, with no local anomaly above 1.2x for either.
Two minutes of the same stretch through each are in the owner's renders
folder as `watch-film4k-variational-4k-60fps.mp4` and
`watch-film4k-variational-unscaled-60fps.mp4`, the A/B his eyes wanted;
`tests/watch.sh` now scales its bitrate with the frame (48 Mbit/s at 4K).

**The recommended file's 4K scaling, on the same film (later the same day).**
`bidirectional-interpolation-variational-propagated-4k.glsl` is the new
recommendation through `scale_shader.py` with every divisor doubled. The
same five segments, the 4K shader above (k4), the unscaled new file (vp) and
its scaling (vp4):

| segment (s) | k4 | vp | vp4 | k4 | vp | vp4 |
|---|---|---|---|---|---|---|
| 600 | 38.87 | 38.47 | **38.75** | 0.9752 | 0.9748 | **0.9751** |
| 2400 | 38.39 | 37.61 | **38.19** | 0.9701 | 0.9687 | **0.9697** |
| 3000 | 30.64 | 29.14 | **30.09** | 0.9565 | 0.9417 | **0.9524** |
| 4200 | 34.72 | 30.32 | **33.03** | 0.9643 | 0.9507 | **0.9604** |
| 4800 | 35.46 | 33.94 | **34.97** | 0.9666 | 0.9615 | **0.9654** |
| mean | 35.62 | 33.90 | **35.01** | 0.9665 | 0.9595 | **0.9646** |

PSNR left, SSIM right. At twice the ladder's size: V1 19.29 -> 53.52, L1 53.64 -> 55.74,
...
Found 2026-09-06 by the owner's eyes on stairs panning vertically in a
film, and then reproduced on the ladder's new period-24 series
(`tests/TESTING.md`): bars of period 24 px, below the 1/16 level's
32-px Nyquist, on a box panning 6 px per frame along the bars' own
period. This build reads **19.33 dB** there, five below frame duplication
and eleven below the stock blend, with every bar present and sharp and
the whole field displaced onto the alias; on hard-edged bars of the same
period 16.60. So does every other two-frame file, seeded or not,
cascade or not, at 19 dB; the four-frame propagated quad reads 50 on the
soft bars and 27 on the hard ones. With the zero seed switched on the propagated file reads 49.55 and the
cascade rebuilt on it 52.72 on the soft bars and 27 to 29 on the hard
ones. Since 2026-09-06 the seed is ON by default in the propagated, seeded
and animation bases (the owner's call: its six ladder losses are the
Fourier-edge and reach cases, a scientific interest, and film is panning,
zooming and translation; the switch stays for that interest) and the
cascade on the propagated base is the recommended file. This build
cannot take the seed, being built on the stock base, which has no
zero-seed code; its 4K sibling is replaced by the recommended file's.
...
A second parameter, `read_alpha`, is the owner's idea (2026-09-06) for the
case the painting could not show. A rotating disc is fastest at its rim and
still at its axis; the hue wheel shows the direction turning, but the
painting saturated everything past 3 px, so the disc read as one uniform
wheel and the gradient of speed across it was invisible. Now the painting's
opacity follows the field's magnitude, and the parameter is a three-pole
switch (his call, 2026-09-07: auto by default, manual for fine tuning):

    read_alpha   0    auto (the default): full opacity at the frame's running
                      maximum of the pooled field, so the fastest thing in view
                      is opaque and the rest in proportion, on any content
                 > 0  manual: full opacity at this many px per frame (a tenth
                      of it for acceleration and jerk, whose gates sit a decade
                      lower); a measurement, like the hue
                 < 0  the flat painting of before, saturating just above the gate

The auto scale is an exposure, found by two small passes behind the pool: a
1/64-res pass takes the largest pooled magnitude in each 8x8 block of cells,
a one-texel pass takes the maximum of those and keeps it in a storage
texel with a memory. A rising maximum is followed at 0.3 per output frame
(70% in three frames); a falling one decays at 0.99 per frame (half-life 69
frames, about 3 s at 24 fps), so a slowing scene fades rather than staying
opaque; and it never drops below the HI gate of the field it reads, so a
still scene amplifies nothing. Constants `READ_AUTO_ATTACK`, `READ_AUTO_DECAY`,
`READ_AUTO_FLOOR_VEL/ACC` in that pass. The manual scale bypasses it; either
way the gate still decides whether a texel is painted, and this decides how
much. In mpv, `--glsl-shader-opts=read_view=1,read_alpha=8`; under ffmpeg
edit the bare number that ends the `//!PARAM read_alpha` block the same way
as `read_view`. For the manual scale, set it by eye, and know that the
pooled reading under-reads a rim: the estimator itself reads the ladder's
spinning disc at omega r to within a few percent out to the edge (mode 4,
frame 47: 6.60 px at a true 7.20 in the outermost band, 1.01 in every
band inside it), but the painting shows the pooled field (mode 7), whose
96 px window averages directions across the disc and reaches past its
edge into the still background, so the same rim band reads 4.31 px, a
ratio of 0.60 that rises to 0.9 by mid-radius and 1.0 at the axis. On that
disc 5 fills the rim; 8 shows the gradient with the rim at half density; a
fast pan wants 16 or more. The owner asked for the same on a large,
ordinary disc (the blob is small and its Fourier edge is itself a
rotational signal): a plain circle of radius 480 px at 1080p with an
irregular texture of periods 36-76 px, its rim at 8.0 px per frame. There
the estimator reads omega r at 0.93-1.03 in every band from r 30 to the
rim, and the pooled reading is 0.89-1.00 out to r 430, falling only inside
the last half window (0.84 at r 430-460, 0.61 at 460-478). So the
under-read is the window reaching past the edge, nothing else: on a disc
larger than a few windows, the manual scale at the rim's speed paints
opacity r / R, which is the display the parameter was asked for. The auto
scale on the same disc puts the rim at 1 whatever its speed, which is the
point of it and also its limit: a disc slowing to a stop stays dense until
the scale's decay catches up, so the physics of a deceleration is the
manual scale's to show.
