...
//!HEIGHT HOOKED.h 16 /
//!COMPONENTS 4
//!DESC [tri] coarse flow search slot1->slot2 and slot2->slot1 (1/16 res) [fused: one dispatch]
...
//!HEIGHT HOOKED.h 2 /
//!COMPONENTS 2
//!DESC [tri] vector median filter on flow slot1->slot2 (pass 1) [fused with its B->A twin: one dispatch]
void hook_ba() {
    if (!pair_changed)
...
// active but the field is near zero" and "my edit did not take effect" stop
// looking the same.
int TRI_DIAG = (read_view == 1 || read_view == 4 || read_view >= 7) ? 7 : (read_view == 2 || read_view == 5) ? 2 : 2;
