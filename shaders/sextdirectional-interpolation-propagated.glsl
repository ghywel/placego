...
//!HEIGHT HOOKED.h 16 /
//!COMPONENTS 1
//!DESC [sext] downsample slot 0 to 1/16 res (luma)
vec4 hook() {
...
//!HEIGHT HOOKED.h 16 /
//!COMPONENTS 1
//!DESC [sext] downsample slot 1 to 1/16 res (luma)
vec4 hook() {
...
//!HEIGHT HOOKED.h 16 /
//!COMPONENTS 1
//!DESC [sext] downsample slot 2 to 1/16 res (luma)
vec4 hook() {
...
//!HEIGHT HOOKED.h 16 /
//!COMPONENTS 1
//!DESC [sext] downsample slot 3 to 1/16 res (luma)
vec4 hook() {
...
//!HEIGHT HOOKED.h 16 /
//!COMPONENTS 1
//!DESC [sext] downsample slot 4 to 1/16 res (luma)
vec4 hook() {
    return vec4(dot(FRAME4_tex(HOOKED_pos).rgb, vec3(0.299, 0.587, 0.114)), 0, 0, 0);
}
//!HOOK FRAME_MIX
//!BIND HOOKED
//!BIND FRAME1
...
//!HEIGHT HOOKED.h 16 /
//!COMPONENTS 1
//!DESC [sext] downsample slot 5 to 1/16 res (luma)
vec4 hook() {
    return vec4(dot(FRAME5_tex(HOOKED_pos).rgb, vec3(0.299, 0.587, 0.114)), 0, 0, 0);
}
...
//!HEIGHT 1
//!COMPONENTS 1
//!DESC [sext] scene-cut statistic, slots 0-1
vec4 hook() {
    const int N = 24;
    float acc = 0.0;
    for (int y = 0; y < N; y++) {
...
//!HEIGHT 1
//!COMPONENTS 1
//!DESC [sext] scene-cut statistic, slots 1-2
vec4 hook() {
    const int N = 24;
    float acc = 0.0;
    for (int y = 0; y < N; y++) {
...
//!HEIGHT 1
//!COMPONENTS 1
//!DESC [sext] scene-cut statistic, slots 2-3
vec4 hook() {
    const int N = 24;
    float acc = 0.0;
    for (int y = 0; y < N; y++) {
...
//!HEIGHT 1
//!COMPONENTS 1
//!DESC [sext] scene-cut statistic, slots 3-4
vec4 hook() {
    const int N = 24;
    float acc = 0.0;
    for (int y = 0; y < N; y++) {
        for (int x = 0; x < N; x++) {
            vec2 uv = (vec2(float(x), float(y)) + 0.5) / float(N);
            acc += abs(LUMA_D_S_tex(uv).r - LUMA_E_S_tex(uv).r);
        }
    }
    return vec4(acc / float(N * N), 0.0, 0.0, 0.0);
}
...
//!HEIGHT 1
//!COMPONENTS 1
//!DESC [sext] scene-cut statistic, slots 4-5
vec4 hook() {
    const int N = 24;
    float acc = 0.0;
    for (int y = 0; y < N; y++) {
        for (int x = 0; x < N; x++) {
            vec2 uv = (vec2(float(x), float(y)) + 0.5) / float(N);
            acc += abs(LUMA_E_S_tex(uv).r - LUMA_F_S_tex(uv).r);
        }
    }
    return vec4(acc / float(N * N), 0.0, 0.0, 0.0);
}
...
//!HEIGHT HOOKED.h 8 /
//!COMPONENTS 1
//!DESC [sext] downsample slot 0 to 1/8 res (luma)
vec4 hook() {
    return vec4(dot(HOOKED_tex(HOOKED_pos).rgb, vec3(0.299, 0.587, 0.114)), 0, 0, 0);
}
...
//!HEIGHT HOOKED.h 8 /
//!COMPONENTS 1
//!DESC [sext] downsample slot 1 to 1/8 res (luma)
vec4 hook() {
    return vec4(dot(FRAME1_tex(HOOKED_pos).rgb, vec3(0.299, 0.587, 0.114)), 0, 0, 0);
}
//!HOOK FRAME_MIX
...
//!HEIGHT HOOKED.h 8 /
//!COMPONENTS 1
//!DESC [sext] downsample slot 2 to 1/8 res (luma)
vec4 hook() {
    return vec4(dot(FRAME2_tex(HOOKED_pos).rgb, vec3(0.299, 0.587, 0.114)), 0, 0, 0);
}
//!HOOK FRAME_MIX
...
//!HEIGHT HOOKED.h 8 /
//!COMPONENTS 1
//!DESC [sext] downsample slot 3 to 1/8 res (luma)
vec4 hook() {
    return vec4(dot(FRAME3_tex(HOOKED_pos).rgb, vec3(0.299, 0.587, 0.114)), 0, 0, 0);
}
//!HOOK FRAME_MIX
...
//!HEIGHT HOOKED.h 8 /
//!COMPONENTS 1
//!DESC [sext] downsample slot 4 to 1/8 res (luma)
vec4 hook() {
    return vec4(dot(FRAME4_tex(HOOKED_pos).rgb, vec3(0.299, 0.587, 0.114)), 0, 0, 0);
}
//!HOOK FRAME_MIX
...
//!HEIGHT HOOKED.h 8 /
//!COMPONENTS 1
//!DESC [sext] downsample slot 5 to 1/8 res (luma)
vec4 hook() {
    return vec4(dot(FRAME5_tex(HOOKED_pos).rgb, vec3(0.299, 0.587, 0.114)), 0, 0, 0);
}
...
//!HEIGHT HOOKED.h 8 /
//!COMPONENTS 2
//!DESC [prop12] contrast-weighted flow propagation AB (pass 2 of 3) [fused with its B->A twin: one dispatch]
...
//!HEIGHT HOOKED.h 8 /
//!COMPONENTS 2
//!DESC [prop12] contrast-weighted flow propagation AB (pass 3 of 3) [fused with its B->A twin: one dispatch]
...
//!HEIGHT HOOKED.h 8 /
//!COMPONENTS 2
//!DESC [prop12] three-way data check AB: propagated vs raw vs zero, ties to consensus on texture only [fused with its B->A twin: one dispatch]

const float PROP_CHECK_MARGIN = 0.1;
const float PROP_FLAT_CONF    = 0.25;
const float PROP_DISAGREE     = 1.5;
...
//!HEIGHT HOOKED.h 4 /
//!COMPONENTS 1
//!DESC [sext] downsample slot 0 to 1/4 res (luma)
vec4 hook() {
    return vec4(dot(HOOKED_tex(HOOKED_pos).rgb, vec3(0.299, 0.587, 0.114)), 0, 0, 0);
}
...
//!HEIGHT HOOKED.h 4 /
//!COMPONENTS 1
//!DESC [sext] downsample slot 1 to 1/4 res (luma)
vec4 hook() {
    return vec4(dot(FRAME1_tex(HOOKED_pos).rgb, vec3(0.299, 0.587, 0.114)), 0, 0, 0);
}
//!HOOK FRAME_MIX
...
//!HEIGHT HOOKED.h 4 /
//!COMPONENTS 1
//!DESC [sext] downsample slot 2 to 1/4 res (luma)
vec4 hook() {
    return vec4(dot(FRAME2_tex(HOOKED_pos).rgb, vec3(0.299, 0.587, 0.114)), 0, 0, 0);
}
//!HOOK FRAME_MIX
...
//!HEIGHT HOOKED.h 4 /
//!COMPONENTS 1
//!DESC [sext] downsample slot 3 to 1/4 res (luma)
vec4 hook() {
    return vec4(dot(FRAME3_tex(HOOKED_pos).rgb, vec3(0.299, 0.587, 0.114)), 0, 0, 0);
}
//!HOOK FRAME_MIX
...
//!HEIGHT HOOKED.h 4 /
//!COMPONENTS 1
//!DESC [sext] downsample slot 4 to 1/4 res (luma)
vec4 hook() {
    return vec4(dot(FRAME4_tex(HOOKED_pos).rgb, vec3(0.299, 0.587, 0.114)), 0, 0, 0);
}
//!HOOK FRAME_MIX
...
//!HEIGHT HOOKED.h 4 /
//!COMPONENTS 1
//!DESC [sext] downsample slot 5 to 1/4 res (luma)
vec4 hook() {
    return vec4(dot(FRAME5_tex(HOOKED_pos).rgb, vec3(0.299, 0.587, 0.114)), 0, 0, 0);
}
...
//!HEIGHT HOOKED.h 2 /
//!COMPONENTS 1
//!DESC [sext] downsample slot 0 to half res (luma)
vec4 hook() {
    return vec4(dot(HOOKED_tex(HOOKED_pos).rgb, vec3(0.299, 0.587, 0.114)), 0, 0, 0);
}
...
//!HEIGHT HOOKED.h 2 /
//!COMPONENTS 1
//!DESC [sext] downsample slot 1 to half res (luma)
vec4 hook() {
    return vec4(dot(FRAME1_tex(HOOKED_pos).rgb, vec3(0.299, 0.587, 0.114)), 0, 0, 0);
}
//!HOOK FRAME_MIX
...
//!HEIGHT HOOKED.h 2 /
//!COMPONENTS 1
//!DESC [sext] downsample slot 2 to 1/2 res (luma)
vec4 hook() {
    return vec4(dot(FRAME2_tex(HOOKED_pos).rgb, vec3(0.299, 0.587, 0.114)), 0, 0, 0);
}
//!HOOK FRAME_MIX
...
//!HEIGHT HOOKED.h 2 /
//!COMPONENTS 1
//!DESC [sext] downsample slot 3 to 1/2 res (luma)
vec4 hook() {
    return vec4(dot(FRAME3_tex(HOOKED_pos).rgb, vec3(0.299, 0.587, 0.114)), 0, 0, 0);
}
//!HOOK FRAME_MIX
...
//!HEIGHT HOOKED.h 2 /
//!COMPONENTS 1
//!DESC [sext] downsample slot 4 to 1/2 res (luma)
vec4 hook() {
    return vec4(dot(FRAME4_tex(HOOKED_pos).rgb, vec3(0.299, 0.587, 0.114)), 0, 0, 0);
}
//!HOOK FRAME_MIX
...
//!HEIGHT HOOKED.h 2 /
//!COMPONENTS 1
//!DESC [sext] downsample slot 5 to 1/2 res (luma)
vec4 hook() {
    return vec4(dot(FRAME5_tex(HOOKED_pos).rgb, vec3(0.299, 0.587, 0.114)), 0, 0, 0);
}
...
// =====================================================================
// SLOT 1 -> SLOT 2 flow chain ([sext], generated). The base's slot-0 ->
...
// =====================================================================
// SLOT 2 -> SLOT 1 flow chain ([sext], generated). Reverse of the above,
...
//!HEIGHT HOOKED.h 16 /
//!COMPONENTS 4
//!DESC [sext] coarse flow search slot1->slot2 and slot2->slot1 (1/16 res) [fused: one dispatch]
...
//!HEIGHT HOOKED.h 8 /
//!COMPONENTS 2
//!DESC [sext] refine flow slot1->slot2 (1/8 res)
...
float frame_diff_s(vec2 uv) {
    ...
    for (int j = -1; j <= 1; j++) {
        for (int i = -1; i <= 1; i++) {
            vec2 c = uv + vec2(float(i), float(j)) * LUMA_A_S_pt;
            d = max(d, abs(LUMA_B_S_tex(c).r - LUMA_C_S_tex(c).r));
        }
    }
    return d;
}
...
//!HEIGHT HOOKED.h 8 /
//!COMPONENTS 2
//!DESC [sext] refine flow slot2->slot1 (1/8 res)
...
//!BIND LUMA_B_E
//!BIND FLOW_E_CB_RAW
//!BIND FLOW_E_CB_PROP_ST1
//!BIND LUMA_C_E
//!SAVE FLOW_E_BC_PROP
//!WIDTH HOOKED.w 8 /
//!HEIGHT HOOKED.h 8 /
//!COMPONENTS 2
//!DESC [prop12] contrast-weighted flow propagation slot1->slot2 (pass 1 of 3) [fused with its B->A twin: one dispatch]
...
void hook_ba() {
    ...
    float w_own = PROP_SELF_WEIGHT * c_own * c_own;
    if (wsum + w_own <= 0.0)
        { imageStore(FLOW_E_CB_PROP_ST1, ivec2(gl_FragCoord.xy), vec4(own, 0.0, 0.0)); return; }
    { imageStore(FLOW_E_CB_PROP_ST1, ivec2(gl_FragCoord.xy), vec4((w_own * own + acc) / (w_own + wsum), 0.0, 0.0)); return; }
}
vec4 hook() {
    hook_ba();
...
//!BIND LUMA_B_E
//!BIND FLOW_E_CB_PROP_ST1
//!BIND FLOW_E_CB_PROP_ST2
//!BIND LUMA_C_E
//!SAVE FLOW_E_BC_PROP
//!WIDTH HOOKED.w 8 /
//!HEIGHT HOOKED.h 8 /
//!COMPONENTS 2
//!DESC [prop12] contrast-weighted flow propagation slot1->slot2 (pass 2 of 3) [fused with its B->A twin: one dispatch]
...
void hook_ba() {
    ...
    vec2 own = imageLoad(FLOW_E_CB_PROP_ST1, ivec2(floor((FLOW_E_BC_PROP_size) * (uv)))).xy;
    float c_own = prop_conf(uv);
    vec2 acc = vec2(0.0);
    float wsum = 0.0;
    for (int y = -2; y <= 2; y++)
        for (int x = -2; x <= 2; x++) {
            ...
            float w = prop_conf(uv + o) / (1.0 + 0.5 * float(abs(x) + abs(y)));
            acc += w * imageLoad(FLOW_E_CB_PROP_ST1, ivec2(floor((FLOW_E_BC_PROP_size) * (uv + o)))).xy;
            wsum += w;
        }
    float w_own = PROP_SELF_WEIGHT * c_own * c_own;
    if (wsum + w_own <= 0.0)
        { imageStore(FLOW_E_CB_PROP_ST2, ivec2(gl_FragCoord.xy), vec4(own, 0.0, 0.0)); return; }
    { imageStore(FLOW_E_CB_PROP_ST2, ivec2(gl_FragCoord.xy), vec4((w_own * own + acc) / (w_own + wsum), 0.0, 0.0)); return; }
}
vec4 hook() {
    hook_ba();
    vec2 uv = FLOW_E_BC_PROP_pos;
...
//!BIND LUMA_B_E
//!BIND FLOW_E_CB_PROP_ST2
//!BIND FLOW_E_CB_PROP_ST3
//!BIND LUMA_C_E
//!SAVE FLOW_E_BC_PROP
//!WIDTH HOOKED.w 8 /
//!HEIGHT HOOKED.h 8 /
//!COMPONENTS 2
//!DESC [prop12] contrast-weighted flow propagation slot1->slot2 (pass 3 of 3) [fused with its B->A twin: one dispatch]
...
void hook_ba() {
    vec2 uv = FLOW_E_BC_PROP_pos;
    vec2 own = imageLoad(FLOW_E_CB_PROP_ST2, ivec2(floor((FLOW_E_BC_PROP_size) * (uv)))).xy;
    float c_own = prop_conf(uv);
    vec2 acc = vec2(0.0);
    float wsum = 0.0;
    for (int y = -2; y <= 2; y++)
        for (int x = -2; x <= 2; x++) {
            ...
            vec2 o = vec2(float(x), float(y)) * FLOW_E_BC_PROP_pt;
            float w = prop_conf(uv + o) / (1.0 + 0.5 * float(abs(x) + abs(y)));
            acc += w * imageLoad(FLOW_E_CB_PROP_ST2, ivec2(floor((FLOW_E_BC_PROP_size) * (uv + o)))).xy;
            wsum += w;
        }
    float w_own = PROP_SELF_WEIGHT * c_own * c_own;
    if (wsum + w_own <= 0.0)
        { imageStore(FLOW_E_CB_PROP_ST3, ivec2(gl_FragCoord.xy), vec4(own, 0.0, 0.0)); return; }
    { imageStore(FLOW_E_CB_PROP_ST3, ivec2(gl_FragCoord.xy), vec4((w_own * own + acc) / (w_own + wsum), 0.0, 0.0)); return; }
}
vec4 hook() {
    hook_ba();
    vec2 uv = FLOW_E_BC_PROP_pos;
...
//!HEIGHT HOOKED.h 8 /
//!COMPONENTS 2
//!DESC [prop12] three-way data check slot1->slot2: propagated vs raw vs zero, ties to consensus on texture only [fused with its B->A twin: one dispatch]
...
void hook_ba() {
    ...
    vec2 prop = imageLoad(FLOW_E_CB_PROP_ST3, ivec2(floor((FLOW_E_BC_PROP_size) * (uv)))).xy;
    if (prop == raw)
        { imageStore(FLOW_E_CB_ST, ivec2(gl_FragCoord.xy), vec4(raw, 0.0, 0.0)); return; }
    float s_prop = sad5_ba(uv, prop * LUMA_B_E_pt);
    float s_raw  = sad5_ba(uv, raw * LUMA_B_E_pt);
    float s_zero = sad5_ba(uv, vec2(0.0));
    float best = min(s_raw, s_zero);
    ...
    if (textured) {
        ...
        // the three-way rule as before.
        if (s_prop < best * (1.0 - PROP_CHECK_MARGIN) - 1e-4)
            { imageStore(FLOW_E_CB_ST, ivec2(gl_FragCoord.xy), vec4(prop, 0.0, 0.0)); return; }
        // prop12: the disagreement threshold scales with the flow, so a fast
        // translation whose consensus is diluted by background votes near its
        // edges is not mistaken for an alias (aliases sit texels apart at
        // speeds of a texel or two).
        if (length(prop - raw) > max(PROP_DISAGREE, PROP_DISAGREE_REL * length(raw)))
            { imageStore(FLOW_E_CB_ST, ivec2(gl_FragCoord.xy), vec4(0.0)); return; }
        if (s_prop <= best * (1.0 + PROP_CHECK_MARGIN) + 1e-4)
            { imageStore(FLOW_E_CB_ST, ivec2(gl_FragCoord.xy), vec4(prop, 0.0, 0.0)); return; }
        { imageStore(FLOW_E_CB_ST, ivec2(gl_FragCoord.xy), s_raw <= s_zero ? vec4(raw, 0.0, 0.0) : vec4(0.0)); return; }
    }
    // flat: evidence required
    if (s_prop < best * (1.0 - PROP_CHECK_MARGIN) - 1e-4)
        { imageStore(FLOW_E_CB_ST, ivec2(gl_FragCoord.xy), vec4(prop, 0.0, 0.0)); return; }
...
//!HEIGHT HOOKED.h 4 /
//!COMPONENTS 2
//!DESC [sext] refine flow slot1->slot2 (1/4 res)
...
//!HEIGHT HOOKED.h 4 /
//!COMPONENTS 2
//!DESC [sext] refine flow slot2->slot1 (1/4 res)
...
//!HEIGHT HOOKED.h 2 /
//!COMPONENTS 2
//!DESC [sext] refine flow slot1->slot2 (half res)
...
//!HEIGHT HOOKED.h 2 /
//!COMPONENTS 2
//!DESC [sext] refine flow slot2->slot1 (half res)
...
//!BIND FLOW_H_BC
//!BIND FLOW_H_CB
//!BIND FLOW_H_CB_M1_ST
//!SAVE FLOW_H_BC
//!WIDTH HOOKED.w 2 /
//!HEIGHT HOOKED.h 2 /
//!COMPONENTS 2
//!DESC [sext] vector median filter on flow slot1->slot2 (pass 1) [fused with its B->A twin: one dispatch]
void hook_ba() {
    if (!pair_changed)
        { imageStore(FLOW_H_CB_M1_ST, ivec2(gl_FragCoord.xy), vec4(0.0)); return; }
    ...
    { imageStore(FLOW_H_CB_M1_ST, ivec2(gl_FragCoord.xy), vec4(best, 0.0, 0.0)); return; }
}
...
//!HEIGHT HOOKED.h 2 /
//!COMPONENTS 2
//!DESC [sext] vector median filter on flow slot1->slot2 (pass 2)
vec4 hook() {
...
//!HEIGHT HOOKED.h 2 /
//!COMPONENTS 2
//!DESC [sext] vector median filter on flow slot2->slot1 (pass 2)
vec4 hook() {
    ivec2 coord = ivec2(FLOW_H_BC_pos * FLOW_H_BC_size);
    if (!pair_changed)
    ...
    for (int y = -1; y <= 1; y++) {
        for (int x = -1; x <= 1; x++) {
            vec2 o = vec2(float(x), float(y)) * FLOW_H_BC_pt;
            v[n++] = imageLoad(FLOW_H_CB_M1_ST, ivec2(floor((FLOW_H_BC_size) * (FLOW_H_BC_pos + o)))).xy;
        }
    }
...
// =====================================================================
// SLOT 2 -> SLOT 3 flow chain ([sext], generated). The pair the fourth
...
// =====================================================================
// SLOT 3 -> SLOT 2 flow chain ([sext], generated). Reverse of the above,
...
//!HEIGHT HOOKED.h 16 /
//!COMPONENTS 4
//!DESC [sext] coarse flow search slot2->slot3 and slot3->slot2 (1/16 res) [fused: one dispatch]
...
//!HEIGHT HOOKED.h 8 /
//!COMPONENTS 2
//!DESC [sext] refine flow slot2->slot3 (1/8 res)
...
float frame_diff_s(vec2 uv) {
    ...
    for (int j = -1; j <= 1; j++) {
        for (int i = -1; i <= 1; i++) {
            vec2 c = uv + vec2(float(i), float(j)) * LUMA_A_S_pt;
            d = max(d, abs(LUMA_C_S_tex(c).r - LUMA_D_S_tex(c).r));
        }
    }
    return d;
}
...
//!HEIGHT HOOKED.h 8 /
//!COMPONENTS 2
//!DESC [sext] refine flow slot3->slot2 (1/8 res)
...
//!HEIGHT HOOKED.h 8 /
//!COMPONENTS 2
//!DESC [prop12] contrast-weighted flow propagation slot2->slot3 (pass 1 of 3) [fused with its B->A twin: one dispatch]
...
void hook_ba() {
    ...
    float w_own = PROP_SELF_WEIGHT * c_own * c_own;
    if (wsum + w_own <= 0.0)
        { imageStore(FLOW_E_DC_PROP_ST1, ivec2(gl_FragCoord.xy), vec4(own, 0.0, 0.0)); return; }
    { imageStore(FLOW_E_DC_PROP_ST1, ivec2(gl_FragCoord.xy), vec4((w_own * own + acc) / (w_own + wsum), 0.0, 0.0)); return; }
}
vec4 hook() {
    hook_ba();
...
//!HEIGHT HOOKED.h 8 /
//!COMPONENTS 2
//!DESC [prop12] contrast-weighted flow propagation slot2->slot3 (pass 2 of 3) [fused with its B->A twin: one dispatch]
...
void hook_ba() {
    ...
    vec2 own = imageLoad(FLOW_E_DC_PROP_ST1, ivec2(floor((FLOW_E_CD_PROP_size) * (uv)))).xy;
    float c_own = prop_conf(uv);
    vec2 acc = vec2(0.0);
    float wsum = 0.0;
    for (int y = -2; y <= 2; y++)
        for (int x = -2; x <= 2; x++) {
            ...
            float w = prop_conf(uv + o) / (1.0 + 0.5 * float(abs(x) + abs(y)));
            acc += w * imageLoad(FLOW_E_DC_PROP_ST1, ivec2(floor((FLOW_E_CD_PROP_size) * (uv + o)))).xy;
            wsum += w;
        }
    float w_own = PROP_SELF_WEIGHT * c_own * c_own;
    if (wsum + w_own <= 0.0)
        { imageStore(FLOW_E_DC_PROP_ST2, ivec2(gl_FragCoord.xy), vec4(own, 0.0, 0.0)); return; }
    { imageStore(FLOW_E_DC_PROP_ST2, ivec2(gl_FragCoord.xy), vec4((w_own * own + acc) / (w_own + wsum), 0.0, 0.0)); return; }
}
vec4 hook() {
    hook_ba();
    vec2 uv = FLOW_E_CD_PROP_pos;
...
//!HEIGHT HOOKED.h 8 /
//!COMPONENTS 2
//!DESC [prop12] contrast-weighted flow propagation slot2->slot3 (pass 3 of 3) [fused with its B->A twin: one dispatch]
...
void hook_ba() {
    vec2 uv = FLOW_E_CD_PROP_pos;
    vec2 own = imageLoad(FLOW_E_DC_PROP_ST2, ivec2(floor((FLOW_E_CD_PROP_size) * (uv)))).xy;
    float c_own = prop_conf(uv);
    vec2 acc = vec2(0.0);
    float wsum = 0.0;
    for (int y = -2; y <= 2; y++)
        for (int x = -2; x <= 2; x++) {
            ...
            vec2 o = vec2(float(x), float(y)) * FLOW_E_CD_PROP_pt;
            float w = prop_conf(uv + o) / (1.0 + 0.5 * float(abs(x) + abs(y)));
            acc += w * imageLoad(FLOW_E_DC_PROP_ST2, ivec2(floor((FLOW_E_CD_PROP_size) * (uv + o)))).xy;
            wsum += w;
        }
    float w_own = PROP_SELF_WEIGHT * c_own * c_own;
    if (wsum + w_own <= 0.0)
        { imageStore(FLOW_E_DC_PROP_ST3, ivec2(gl_FragCoord.xy), vec4(own, 0.0, 0.0)); return; }
    { imageStore(FLOW_E_DC_PROP_ST3, ivec2(gl_FragCoord.xy), vec4((w_own * own + acc) / (w_own + wsum), 0.0, 0.0)); return; }
}
vec4 hook() {
    hook_ba();
    vec2 uv = FLOW_E_CD_PROP_pos;
...
//!HEIGHT HOOKED.h 8 /
//!COMPONENTS 2
//!DESC [prop12] three-way data check slot2->slot3: propagated vs raw vs zero, ties to consensus on texture only [fused with its B->A twin: one dispatch]
...
void hook_ba() {
    ...
    vec2 prop = imageLoad(FLOW_E_DC_PROP_ST3, ivec2(floor((FLOW_E_CD_PROP_size) * (uv)))).xy;
    if (prop == raw)
        { imageStore(FLOW_E_DC_ST, ivec2(gl_FragCoord.xy), vec4(raw, 0.0, 0.0)); return; }
    float s_prop = sad5_ba(uv, prop * LUMA_B_E_pt);
    float s_raw  = sad5_ba(uv, raw * LUMA_B_E_pt);
    float s_zero = sad5_ba(uv, vec2(0.0));
    float best = min(s_raw, s_zero);
    ...
    if (textured) {
        ...
        // the three-way rule as before.
        if (s_prop < best * (1.0 - PROP_CHECK_MARGIN) - 1e-4)
            { imageStore(FLOW_E_DC_ST, ivec2(gl_FragCoord.xy), vec4(prop, 0.0, 0.0)); return; }
        // prop12: the disagreement threshold scales with the flow, so a fast
        // translation whose consensus is diluted by background votes near its
        // edges is not mistaken for an alias (aliases sit texels apart at
        // speeds of a texel or two).
        if (length(prop - raw) > max(PROP_DISAGREE, PROP_DISAGREE_REL * length(raw)))
            { imageStore(FLOW_E_DC_ST, ivec2(gl_FragCoord.xy), vec4(0.0)); return; }
        if (s_prop <= best * (1.0 + PROP_CHECK_MARGIN) + 1e-4)
            { imageStore(FLOW_E_DC_ST, ivec2(gl_FragCoord.xy), vec4(prop, 0.0, 0.0)); return; }
        { imageStore(FLOW_E_DC_ST, ivec2(gl_FragCoord.xy), s_raw <= s_zero ? vec4(raw, 0.0, 0.0) : vec4(0.0)); return; }
    }
    // flat: evidence required
    if (s_prop < best * (1.0 - PROP_CHECK_MARGIN) - 1e-4)
        { imageStore(FLOW_E_DC_ST, ivec2(gl_FragCoord.xy), vec4(prop, 0.0, 0.0)); return; }
...
//!HEIGHT HOOKED.h 4 /
//!COMPONENTS 2
//!DESC [sext] refine flow slot2->slot3 (1/4 res)
...
//!HEIGHT HOOKED.h 4 /
//!COMPONENTS 2
//!DESC [sext] refine flow slot3->slot2 (1/4 res)
...
//!HEIGHT HOOKED.h 2 /
//!COMPONENTS 2
//!DESC [sext] refine flow slot2->slot3 (half res)
...
//!HEIGHT HOOKED.h 2 /
//!COMPONENTS 2
//!DESC [sext] refine flow slot3->slot2 (half res)
...
//!BIND FLOW_H_CD
//!BIND FLOW_H_DC
//!BIND FLOW_H_DC_M1_ST
//!SAVE FLOW_H_CD
//!WIDTH HOOKED.w 2 /
//!HEIGHT HOOKED.h 2 /
//!COMPONENTS 2
//!DESC [sext] vector median filter on flow slot2->slot3 (pass 1) [fused with its B->A twin: one dispatch]
void hook_ba() {
    if (!pair_changed)
        { imageStore(FLOW_H_DC_M1_ST, ivec2(gl_FragCoord.xy), vec4(0.0)); return; }
    ...
    { imageStore(FLOW_H_DC_M1_ST, ivec2(gl_FragCoord.xy), vec4(best, 0.0, 0.0)); return; }
}
...
//!HEIGHT HOOKED.h 2 /
//!COMPONENTS 2
//!DESC [sext] vector median filter on flow slot2->slot3 (pass 2)
vec4 hook() {
...
//!HEIGHT HOOKED.h 2 /
//!COMPONENTS 2
//!DESC [sext] vector median filter on flow slot3->slot2 (pass 2)
vec4 hook() {
    ivec2 coord = ivec2(FLOW_H_CD_pos * FLOW_H_CD_size);
    if (!pair_changed)
    ...
    for (int y = -1; y <= 1; y++) {
        for (int x = -1; x <= 1; x++) {
            vec2 o = vec2(float(x), float(y)) * FLOW_H_CD_pt;
            v[n++] = imageLoad(FLOW_H_DC_M1_ST, ivec2(floor((FLOW_H_CD_size) * (FLOW_H_CD_pos + o)))).xy;
        }
    }
...
// =====================================================================
// SLOT 3 -> SLOT 4 flow chain ([sext], generated). The pair the fifth
// frame adds: the second link of the composed +2 displacement
// F24(x) = F23(x) + F34(x + F23(x)), the mirror of the quad's far flow.
// =====================================================================

// =====================================================================
// SLOT 4 -> SLOT 3 flow chain ([sext], generated). Reverse of the above,
...
// see the "Storage-based flow caching" note at the top of this file.
// ---------------------------------------------------------------------
//!TEXTURE FLOW_S_DE_CACHE
//!SIZE 240 135
//!FORMAT rgba32f
//!STORAGE

//!TEXTURE FLOW_S_DE_CACHE2
//!SIZE 240 135
//!FORMAT rgba32f
//!STORAGE

//!TEXTURE FLOW_S_ED_CACHE
//!SIZE 240 135
//!FORMAT rgba32f
//!STORAGE

//!TEXTURE FLOW_S_ED_CACHE2
//!SIZE 240 135
//!FORMAT rgba32f
//!STORAGE

//!HOOK FRAME_MIX
//!BIND FLOW_S_DE_CACHE
//!BIND FLOW_S_DE_CACHE2
//!BIND FLOW_S_ED_CACHE
//!BIND FLOW_S_ED_CACHE2
//!BIND LUMA_A_S
//!BIND LUMA_B_S
//!BIND LUMA_D_S
//!BIND LUMA_E_S
...
//!HEIGHT HOOKED.h 16 /
//!COMPONENTS 4
//!DESC [sext] coarse flow search slot3->slot4 and slot4->slot3 (1/16 res) [fused: one dispatch]
...
float sad5x5_s(vec2 uv_a, vec2 uv_b) {
    ...
    for (int y = -COARSE_WINDOW_RADIUS; y <= COARSE_WINDOW_RADIUS; y++) {
        for (int x = -COARSE_WINDOW_RADIUS; x <= COARSE_WINDOW_RADIUS; x++) {
            vec2 o = vec2(float(x), float(y)) * LUMA_A_S_pt;
            s += abs(LUMA_D_S_tex(uv_a + o).r - LUMA_E_S_tex(uv_b + o).r);
        }
    }
    return s;
}
...
float local_contrast_5x5_s(vec2 uv_a) {
    ...
    for (int y = -2; y <= 2; y++) {
        for (int x = -2; x <= 2; x++) {
            float v = LUMA_D_S_tex(uv_a + vec2(float(x), float(y)) * LUMA_A_S_pt).r;
            lo = min(lo, v);
            hi = max(hi, v);
        }
    }
...
float sad5x5_s2(vec2 uv_b, vec2 uv_a) {
    ...
    for (int y = -COARSE_WINDOW_RADIUS; y <= COARSE_WINDOW_RADIUS; y++) {
        for (int x = -COARSE_WINDOW_RADIUS; x <= COARSE_WINDOW_RADIUS; x++) {
            vec2 o = vec2(float(x), float(y)) * LUMA_A_S_pt;
            s += abs(LUMA_E_S_tex(uv_b + o).r - LUMA_D_S_tex(uv_a + o).r);
        }
    }
    return s;
}
float local_contrast_5x5_s2(vec2 uv_b) {
    ...
    for (int y = -2; y <= 2; y++) {
        for (int x = -2; x <= 2; x++) {
            float v = LUMA_E_S_tex(uv_b + vec2(float(x), float(y)) * LUMA_A_S_pt).r;
            lo = min(lo, v);
            hi = max(hi, v);
        }
    }
...
void coarse_ba() {
    ...
    vec2 uv_b = LUMA_A_S_pos;
    vec2 prev_s = imageLoad(FLOW_S_ED_CACHE, coord).xy * LUMA_A_S_pt;

    if (local_contrast_5x5_s2(uv_b) < MIN_CONTRAST) {
        vec4 result = vec4(0.0);
        imageStore(FLOW_S_ED_CACHE, coord, result);
        imageStore(FLOW_S_ED_CACHE2, coord, result);
        return;
    }
    ...
    vec2 off_b = descend_s2(uv_b, start_b, cost_b);
    vec2 off_c = descend_s2(uv_b, prev_s, cost_c);
    imageStore(FLOW_S_ED_CACHE2, coord, vec4(off_c / LUMA_A_S_pt, 0.0, 0.0));
    vec4 result = vec4(off_a / LUMA_A_S_pt, off_b / LUMA_A_S_pt);
    imageStore(FLOW_S_ED_CACHE, coord, result);
}
vec4 hook() {
    ivec2 coord = ivec2(LUMA_A_S_pos * LUMA_A_S_size);
    if (!pair_changed)
        return imageLoad(FLOW_S_DE_CACHE, coord);
    coarse_ba();

    vec2 uv_a = LUMA_A_S_pos;
    vec2 prev_s = imageLoad(FLOW_S_DE_CACHE, coord).xy * LUMA_A_S_pt;

    if (local_contrast_5x5_s(uv_a) < MIN_CONTRAST) {
        vec4 result = vec4(0.0);
        imageStore(FLOW_S_DE_CACHE, coord, result);
        imageStore(FLOW_S_DE_CACHE2, coord, result);
        return result;
    }
    ...
    vec2 off_b = descend_s(uv_a, start_b, cost_b);
    vec2 off_c = descend_s(uv_a, prev_s, cost_c);
    imageStore(FLOW_S_DE_CACHE2, coord, vec4(off_c / LUMA_A_S_pt, 0.0, 0.0));
    vec4 result = vec4(off_a / LUMA_A_S_pt, off_b / LUMA_A_S_pt);
    imageStore(FLOW_S_DE_CACHE, coord, result);
    return result;
}
...
//!TEXTURE FLOW_E_DE_CACHE
//!SIZE 480 270
//!FORMAT rgba32f
//!STORAGE

//!HOOK FRAME_MIX
//!BIND FLOW_E_DE_CACHE
//!BIND LUMA_A_E
//!BIND LUMA_B_E
//!BIND FLOW_S_DE
//!BIND FLOW_S_DE_CACHE2
//!BIND LUMA_A_S
//!BIND FLOW_E_ED_CACHE
//!BIND LUMA_B_S
//!BIND LUMA_D_S
//!BIND LUMA_E_S
//!BIND LUMA_D_E
...
//!HEIGHT HOOKED.h 8 /
//!COMPONENTS 2
//!DESC [sext] refine flow slot3->slot4 (1/8 res)

// Snaps to the exact center of whichever FLOW_S_DE texel this position
// falls in, before reading it as this level's search seed just below.
// An ordinary bilinear read here (as this used to be) blends between
// neighboring coarse-level vectors whenever the sample position isn't
// exactly on a coarse texel center -- which is most positions, since
...
// texel holds the object's true motion and its neighbor holds ~zero,
// that blend produces a smooth gradient of in-between seed vectors
// spanning roughly one full FLOW_S_DE texel width in every direction --
// 16 full-res pixels each way at this handoff specifically -- entirely
// independent of how correct the underlying FLOW_S_DE values are.
// Real-hardware testing already ruled out the coarse search's own
// matching-window size as the (sole) cause of the "nebulous cloud" seen
// bleeding from real motion boundaries into neighboring static content
// in interpolate-debug-overlay.glsl (see COARSE_WINDOW_RADIUS in the
...
float sad5x5_e(vec2 uv_a, vec2 uv_b) {
    ...
    for (int y = -COARSE_WINDOW_RADIUS; y <= COARSE_WINDOW_RADIUS; y++) {
        for (int x = -COARSE_WINDOW_RADIUS; x <= COARSE_WINDOW_RADIUS; x++) {
            vec2 o = vec2(float(x), float(y)) * LUMA_A_E_pt;
            s += abs(LUMA_D_E_tex(uv_a + o).r - LUMA_E_E_tex(uv_b + o).r);
        }
    }
    return s;
}
...
float local_contrast_5x5_e(vec2 uv_a) {
    ...
    for (int y = -2; y <= 2; y++) {
        for (int x = -2; x <= 2; x++) {
            float v = LUMA_D_E_tex(uv_a + vec2(float(x), float(y)) * LUMA_A_E_pt).r;
            lo = min(lo, v);
            hi = max(hi, v);
        }
    }
...
float frame_diff_s(vec2 uv) {
    ...
    for (int j = -1; j <= 1; j++) {
        for (int i = -1; i <= 1; i++) {
            vec2 c = uv + vec2(float(i), float(j)) * LUMA_A_S_pt;
            d = max(d, abs(LUMA_D_S_tex(c).r - LUMA_E_S_tex(c).r));
        }
    }
    return d;
}
...
float moire_s(vec2 uv) {
    ...
    for (int j = -1; j <= 1; j++) {
        for (int i = -1; i <= 1; i++) {
            vec2 c = uv + vec2(float(i), float(j)) * LUMA_A_S_pt;
            float p = LUMA_D_S_tex(c).r;
            float b = 0.25 * (LUMA_D_E_tex(c + vec2(-0.25, -0.25) * LUMA_A_S_pt).r + LUMA_D_E_tex(c + vec2(0.25, -0.25) * LUMA_A_S_pt).r
                            + LUMA_D_E_tex(c + vec2(-0.25, 0.25) * LUMA_A_S_pt).r + LUMA_D_E_tex(c + vec2(0.25, 0.25) * LUMA_A_S_pt).r);
            plo = min(plo, p); phi = max(phi, p); blo = min(blo, b); bhi = max(bhi, b);
        }
    }
    float cp = phi - plo, cb = bhi - blo;
...
vec4 hook() {
    ivec2 coord = ivec2(LUMA_A_E_pos * LUMA_A_E_size);
    if (!pair_changed)
        return imageLoad(FLOW_E_DE_CACHE, coord);
    ...
    // simply not be enough room for them to do that job.
    // ---- refine three seeds; the temporal seed and prior only where the previous flow round-trips ----
    vec2 prev_e = imageLoad(FLOW_E_DE_CACHE, coord).xy * LUMA_A_E_pt;
    ivec2 rcoord = clamp(ivec2((uv_a + prev_e) * LUMA_A_E_size), ivec2(0), ivec2(LUMA_A_E_size) - 1);
    vec2 prev_rev = imageLoad(FLOW_E_ED_CACHE, rcoord).xy * LUMA_A_E_pt;
    float rt = length((prev_e + prev_rev) / LUMA_A_E_pt);
    bool trusted = rt < SEED_RT_MAX && length(prev_e) > 0.0;
    float tl = trusted ? SEED_TEMP_LAMBDA : 0.0;
    ivec2 scoord = ivec2(snap_texel(uv_a, FLOW_S_DE_size) * FLOW_S_DE_size);
    vec2 base_off3 = imageLoad(FLOW_S_DE_CACHE2, scoord).xy * 2.0 * LUMA_A_E_pt;
    float sad_a, sad_b, sad_c;
    vec2 ref_a = refine_e(uv_a, base_off, sad_a);
    vec2 ref_b = refine_e(uv_a, base_off2, sad_b);
    vec2 ref_c = refine_e(uv_a, base_off3, sad_c);
...
//!HEIGHT HOOKED.h 8 /
//!COMPONENTS 2
//!DESC [sext] refine flow slot4->slot3 (1/8 res)
...
float sad5x5_e2(vec2 uv_b, vec2 uv_a) {
    ...
    for (int y = -COARSE_WINDOW_RADIUS; y <= COARSE_WINDOW_RADIUS; y++) {
        for (int x = -COARSE_WINDOW_RADIUS; x <= COARSE_WINDOW_RADIUS; x++) {
            vec2 o = vec2(float(x), float(y)) * LUMA_A_E_pt;
            s += abs(LUMA_E_E_tex(uv_b + o).r - LUMA_D_E_tex(uv_a + o).r);
        }
    }
    return s;
}
...
float local_contrast_5x5_e2(vec2 uv_b) {
    ...
    for (int y = -2; y <= 2; y++) {
        for (int x = -2; x <= 2; x++) {
            float v = LUMA_E_E_tex(uv_b + vec2(float(x), float(y)) * LUMA_A_E_pt).r;
            lo = min(lo, v);
            hi = max(hi, v);
        }
    }
...
float moire_s(vec2 uv) {
    ...
    for (int j = -1; j <= 1; j++) {
        for (int i = -1; i <= 1; i++) {
            vec2 c = uv + vec2(float(i), float(j)) * LUMA_B_S_pt;
            float p = LUMA_E_S_tex(c).r;
            float b = 0.25 * (LUMA_E_E_tex(c + vec2(-0.25, -0.25) * LUMA_B_S_pt).r + LUMA_E_E_tex(c + vec2(0.25, -0.25) * LUMA_B_S_pt).r
                            + LUMA_E_E_tex(c + vec2(-0.25, 0.25) * LUMA_B_S_pt).r + LUMA_E_E_tex(c + vec2(0.25, 0.25) * LUMA_B_S_pt).r);
            plo = min(plo, p); phi = max(phi, p); blo = min(blo, b); bhi = max(bhi, b);
        }
    }
    float cp = phi - plo, cb = bhi - blo;
...
bool aperture_ok(vec2 uv, vec2 off) {
    ...
    for (int j = -1; j <= 1; j++) {
        for (int i = -1; i <= 1; i++) {
            vec2 c = uv + vec2(float(i), float(j)) * LUMA_A_E_pt;
            float gx = LUMA_E_E_tex(c + vec2(LUMA_A_E_pt.x, 0.0)).r - LUMA_E_E_tex(c - vec2(LUMA_A_E_pt.x, 0.0)).r;
            float gy = LUMA_E_E_tex(c + vec2(0.0, LUMA_A_E_pt.y)).r - LUMA_E_E_tex(c - vec2(0.0, LUMA_A_E_pt.y)).r;
            jxx += gx * gx; jyy += gy * gy; jxy += gx * gy;
        }
    }
    float tr = jxx + jyy, det = jxx * jyy - jxy * jxy;
...
vec4 hook() {
    ivec2 coord = ivec2(LUMA_B_E_pos * LUMA_B_E_size);
    if (!pair_changed)
        return imageLoad(FLOW_E_ED_CACHE, coord);
    ...
    // See REFINE_SEARCH_RADIUS/REFINE_REG_LAMBDA in the A->B pass above.
    // ---- refine three seeds; the temporal seed and prior only where the previous flow round-trips ----
    vec2 prev_e = imageLoad(FLOW_E_ED_CACHE, coord).xy * LUMA_A_E_pt;
    ivec2 rcoord = clamp(ivec2((uv_b + prev_e) * LUMA_A_E_size), ivec2(0), ivec2(LUMA_A_E_size) - 1);
    vec2 prev_rev = imageLoad(FLOW_E_DE_CACHE, rcoord).xy * LUMA_A_E_pt;
    float rt = length((prev_e + prev_rev) / LUMA_A_E_pt);
    bool trusted = rt < SEED_RT_MAX && length(prev_e) > 0.0;
    float tl = trusted ? SEED_TEMP_LAMBDA : 0.0;
    ivec2 scoord = ivec2(snap_texel(uv_b, LUMA_B_S_size) * LUMA_B_S_size);
    vec2 base_off3 = imageLoad(FLOW_S_ED_CACHE2, scoord).xy * 2.0 * LUMA_A_E_pt;
    float sad_a, sad_b, sad_c;
    vec2 ref_a = refine_e(uv_b, base_off, sad_a);
    vec2 ref_b = refine_e(uv_b, base_off2, sad_b);
    vec2 ref_c = refine_e(uv_b, base_off3, sad_c);
...
//!HEIGHT HOOKED.h 8 /
//!COMPONENTS 2
//!DESC [prop12] contrast-weighted flow propagation slot3->slot4 (pass 1 of 3) [fused with its B->A twin: one dispatch]
...
float prop_conf(vec2 uv) {
    ...
    for (int y = -2; y <= 2; y++)
        for (int x = -2; x <= 2; x++) {
            float l = LUMA_D_E_tex(uv + vec2(float(x), float(y)) * LUMA_A_E_pt).r;
            lo = min(lo, l); hi = max(hi, l);
        }
    return clamp((hi - lo) / PROP_CONF_FULL, 0.0, 1.0);
}
...
void hook_ba() {
    vec2 uv = FLOW_E_ED_RAW_pos;
    vec2 own = FLOW_E_ED_RAW_tex(uv).xy;
    float c_own = prop_conf(uv);
    vec2 acc = vec2(0.0);
    float wsum = 0.0;
    for (int y = -2; y <= 2; y++)
        for (int x = -2; x <= 2; x++) {
            if (x == 0 && y == 0) continue;
            vec2 o = vec2(float(x), float(y)) * FLOW_E_ED_RAW_pt;
            float w = prop_conf(uv + o) / (1.0 + 0.5 * float(abs(x) + abs(y)));
            acc += w * FLOW_E_ED_RAW_tex(uv + o).xy;
            wsum += w;
        }
    float w_own = PROP_SELF_WEIGHT * c_own * c_own;
    if (wsum + w_own <= 0.0)
        { imageStore(FLOW_E_ED_PROP_ST1, ivec2(gl_FragCoord.xy), vec4(own, 0.0, 0.0)); return; }
    { imageStore(FLOW_E_ED_PROP_ST1, ivec2(gl_FragCoord.xy), vec4((w_own * own + acc) / (w_own + wsum), 0.0, 0.0)); return; }
}
vec4 hook() {
    hook_ba();
    vec2 uv = FLOW_E_DE_RAW_pos;
    vec2 own = FLOW_E_DE_RAW_tex(uv).xy;
    float c_own = prop_conf(uv);
    vec2 acc = vec2(0.0);
    float wsum = 0.0;
    for (int y = -2; y <= 2; y++)
        for (int x = -2; x <= 2; x++) {
            if (x == 0 && y == 0) continue;
            vec2 o = vec2(float(x), float(y)) * FLOW_E_DE_RAW_pt;
            float w = prop_conf(uv + o) / (1.0 + 0.5 * float(abs(x) + abs(y)));
            acc += w * FLOW_E_DE_RAW_tex(uv + o).xy;
            wsum += w;
        }
    float w_own = PROP_SELF_WEIGHT * c_own * c_own;
    if (wsum + w_own <= 0.0)
...
//!HEIGHT HOOKED.h 8 /
//!COMPONENTS 2
//!DESC [prop12] contrast-weighted flow propagation slot3->slot4 (pass 2 of 3) [fused with its B->A twin: one dispatch]
...
void hook_ba() {
    vec2 uv = FLOW_E_DE_PROP_pos;
    vec2 own = imageLoad(FLOW_E_ED_PROP_ST1, ivec2(floor((FLOW_E_DE_PROP_size) * (uv)))).xy;
    float c_own = prop_conf(uv);
    vec2 acc = vec2(0.0);
    float wsum = 0.0;
    for (int y = -2; y <= 2; y++)
        for (int x = -2; x <= 2; x++) {
            if (x == 0 && y == 0) continue;
            vec2 o = vec2(float(x), float(y)) * FLOW_E_DE_PROP_pt;
            float w = prop_conf(uv + o) / (1.0 + 0.5 * float(abs(x) + abs(y)));
            acc += w * imageLoad(FLOW_E_ED_PROP_ST1, ivec2(floor((FLOW_E_DE_PROP_size) * (uv + o)))).xy;
            wsum += w;
        }
    float w_own = PROP_SELF_WEIGHT * c_own * c_own;
    if (wsum + w_own <= 0.0)
        { imageStore(FLOW_E_ED_PROP_ST2, ivec2(gl_FragCoord.xy), vec4(own, 0.0, 0.0)); return; }
    { imageStore(FLOW_E_ED_PROP_ST2, ivec2(gl_FragCoord.xy), vec4((w_own * own + acc) / (w_own + wsum), 0.0, 0.0)); return; }
}
vec4 hook() {
    hook_ba();
    vec2 uv = FLOW_E_DE_PROP_pos;
    vec2 own = FLOW_E_DE_PROP_tex(uv).xy;
    float c_own = prop_conf(uv);
    vec2 acc = vec2(0.0);
    float wsum = 0.0;
    for (int y = -2; y <= 2; y++)
        for (int x = -2; x <= 2; x++) {
            ...
            vec2 o = vec2(float(x), float(y)) * FLOW_E_DE_PROP_pt;
            float w = prop_conf(uv + o) / (1.0 + 0.5 * float(abs(x) + abs(y)));
            acc += w * FLOW_E_DE_PROP_tex(uv + o).xy;
            wsum += w;
        }
    float w_own = PROP_SELF_WEIGHT * c_own * c_own;
    if (wsum + w_own <= 0.0)
...
//!HEIGHT HOOKED.h 8 /
//!COMPONENTS 2
//!DESC [prop12] contrast-weighted flow propagation slot3->slot4 (pass 3 of 3) [fused with its B->A twin: one dispatch]
...
void hook_ba() {
    vec2 uv = FLOW_E_DE_PROP_pos;
    vec2 own = imageLoad(FLOW_E_ED_PROP_ST2, ivec2(floor((FLOW_E_DE_PROP_size) * (uv)))).xy;
    float c_own = prop_conf(uv);
    vec2 acc = vec2(0.0);
    float wsum = 0.0;
    for (int y = -2; y <= 2; y++)
        for (int x = -2; x <= 2; x++) {
            ...
            vec2 o = vec2(float(x), float(y)) * FLOW_E_DE_PROP_pt;
            float w = prop_conf(uv + o) / (1.0 + 0.5 * float(abs(x) + abs(y)));
            acc += w * imageLoad(FLOW_E_ED_PROP_ST2, ivec2(floor((FLOW_E_DE_PROP_size) * (uv + o)))).xy;
            wsum += w;
        }
    float w_own = PROP_SELF_WEIGHT * c_own * c_own;
    if (wsum + w_own <= 0.0)
        { imageStore(FLOW_E_ED_PROP_ST3, ivec2(gl_FragCoord.xy), vec4(own, 0.0, 0.0)); return; }
    { imageStore(FLOW_E_ED_PROP_ST3, ivec2(gl_FragCoord.xy), vec4((w_own * own + acc) / (w_own + wsum), 0.0, 0.0)); return; }
}
vec4 hook() {
    hook_ba();
    vec2 uv = FLOW_E_DE_PROP_pos;
...
//!HEIGHT HOOKED.h 8 /
//!COMPONENTS 2
//!DESC [prop12] three-way data check slot3->slot4: propagated vs raw vs zero, ties to consensus on texture only [fused with its B->A twin: one dispatch]
...
float sad5(vec2 uv, vec2 flow_uv) {
    ...
    for (int y = -2; y <= 2; y++)
        for (int x = -2; x <= 2; x++) {
            vec2 o = vec2(float(x), float(y)) * LUMA_A_E_pt;
            s += abs(LUMA_D_E_tex(uv + o).r - LUMA_E_E_tex(uv + o + flow_uv).r);
        }
    return s;
}
...
float prop_conf_ba(vec2 uv) {
    ...
    for (int y = -2; y <= 2; y++)
        for (int x = -2; x <= 2; x++) {
            float l = LUMA_E_E_tex(uv + vec2(float(x), float(y)) * LUMA_B_E_pt).r;
            lo = min(lo, l); hi = max(hi, l);
        }
    return clamp((hi - lo) / PROP_CONF_FULL, 0.0, 1.0);
}
...
float sad5_ba(vec2 uv, vec2 flow_uv) {
    ...
    for (int y = -2; y <= 2; y++)
        for (int x = -2; x <= 2; x++) {
            vec2 o = vec2(float(x), float(y)) * LUMA_B_E_pt;
            s += abs(LUMA_E_E_tex(uv + o).r - LUMA_D_E_tex(uv + o + flow_uv).r);
        }
    return s;
}
...
void hook_ba() {
    vec2 uv = FLOW_E_DE_PROP_pos;
    vec2 raw = FLOW_E_ED_RAW_tex(uv).xy;
    vec2 prop = imageLoad(FLOW_E_ED_PROP_ST3, ivec2(floor((FLOW_E_DE_PROP_size) * (uv)))).xy;
    if (prop == raw)
        { imageStore(FLOW_E_ED_ST, ivec2(gl_FragCoord.xy), vec4(raw, 0.0, 0.0)); return; }
    float s_prop = sad5_ba(uv, prop * LUMA_B_E_pt);
    float s_raw  = sad5_ba(uv, raw * LUMA_B_E_pt);
    float s_zero = sad5_ba(uv, vec2(0.0));
    float best = min(s_raw, s_zero);
    ...
    if (textured) {
        ...
        // the three-way rule as before.
        if (s_prop < best * (1.0 - PROP_CHECK_MARGIN) - 1e-4)
            { imageStore(FLOW_E_ED_ST, ivec2(gl_FragCoord.xy), vec4(prop, 0.0, 0.0)); return; }
        // prop12: the disagreement threshold scales with the flow, so a fast
        // translation whose consensus is diluted by background votes near its
        // edges is not mistaken for an alias (aliases sit texels apart at
        // speeds of a texel or two).
        if (length(prop - raw) > max(PROP_DISAGREE, PROP_DISAGREE_REL * length(raw)))
            { imageStore(FLOW_E_ED_ST, ivec2(gl_FragCoord.xy), vec4(0.0)); return; }
        if (s_prop <= best * (1.0 + PROP_CHECK_MARGIN) + 1e-4)
            { imageStore(FLOW_E_ED_ST, ivec2(gl_FragCoord.xy), vec4(prop, 0.0, 0.0)); return; }
        { imageStore(FLOW_E_ED_ST, ivec2(gl_FragCoord.xy), s_raw <= s_zero ? vec4(raw, 0.0, 0.0) : vec4(0.0)); return; }
    }
    // flat: evidence required
    if (s_prop < best * (1.0 - PROP_CHECK_MARGIN) - 1e-4)
        { imageStore(FLOW_E_ED_ST, ivec2(gl_FragCoord.xy), vec4(prop, 0.0, 0.0)); return; }
...
vec4 hook() {
    hook_ba();
    vec2 uv = FLOW_E_DE_PROP_pos;
    vec2 raw = FLOW_E_DE_RAW_tex(uv).xy;
    vec2 prop = FLOW_E_DE_PROP_tex(uv).xy;
    if (prop == raw)
        return vec4(raw, 0.0, 0.0);
    float s_prop = sad5(uv, prop * LUMA_A_E_pt);
    float s_raw  = sad5(uv, raw * LUMA_A_E_pt);
...
//!TEXTURE FLOW_Q_DE_CACHE
//!SIZE 960 540
//!FORMAT rgba32f
//!STORAGE

//!HOOK FRAME_MIX
//!BIND FLOW_Q_DE_CACHE
//!BIND LUMA_A_Q
//!BIND LUMA_B_Q
//!BIND FLOW_E_DE
//!BIND LUMA_D_Q
...
//!HEIGHT HOOKED.h 4 /
//!COMPONENTS 2
//!DESC [sext] refine flow slot3->slot4 (1/4 res)

// Same seed-snapping fix as the S->E handoff above (see that pass for
// the full reasoning) -- here for the E->Q handoff: FLOW_E_DE's own
// texel is 8 full-res px, so an unsnapped bilinear read would smear a
// real boundary across ~8px in every direction (~16px total) when
// seeding this level, on top of whatever the S->E handoff already did.
vec2 snap_texel(vec2 uv, vec2 size) {
...
float sad5x5_q(vec2 uv_a, vec2 uv_b) {
    ...
    for (int y = -COARSE_WINDOW_RADIUS; y <= COARSE_WINDOW_RADIUS; y++) {
        for (int x = -COARSE_WINDOW_RADIUS; x <= COARSE_WINDOW_RADIUS; x++) {
            vec2 o = vec2(float(x), float(y)) * LUMA_A_Q_pt;
            s += abs(LUMA_D_Q_tex(uv_a + o).r - LUMA_E_Q_tex(uv_b + o).r);
        }
    }
    return s;
}
...
float local_contrast_5x5_q(vec2 uv_a) {
    ...
    for (int y = -2; y <= 2; y++) {
        for (int x = -2; x <= 2; x++) {
            float v = LUMA_D_Q_tex(uv_a + vec2(float(x), float(y)) * LUMA_A_Q_pt).r;
            lo = min(lo, v);
            hi = max(hi, v);
        }
    }
...
vec4 hook() {
    ivec2 coord = ivec2(LUMA_A_Q_pos * LUMA_A_Q_size);
    if (!pair_changed)
        return imageLoad(FLOW_Q_DE_CACHE, coord);
...
//!TEXTURE FLOW_Q_ED_CACHE
//!SIZE 960 540
//!FORMAT rgba32f
//!STORAGE

//!HOOK FRAME_MIX
//!BIND FLOW_Q_ED_CACHE
//!BIND LUMA_A_Q
//!BIND LUMA_B_Q
//!BIND FLOW_E_ED_ST
//!BIND FLOW_E_DE
...
//!HEIGHT HOOKED.h 4 /
//!COMPONENTS 2
//!DESC [sext] refine flow slot4->slot3 (1/4 res)
...
float sad5x5_q2(vec2 uv_b, vec2 uv_a) {
    ...
    for (int y = -COARSE_WINDOW_RADIUS; y <= COARSE_WINDOW_RADIUS; y++) {
        for (int x = -COARSE_WINDOW_RADIUS; x <= COARSE_WINDOW_RADIUS; x++) {
            vec2 o = vec2(float(x), float(y)) * LUMA_A_Q_pt;
            s += abs(LUMA_E_Q_tex(uv_b + o).r - LUMA_D_Q_tex(uv_a + o).r);
        }
    }
    return s;
}
...
float local_contrast_5x5_q2(vec2 uv_b) {
    ...
    for (int y = -2; y <= 2; y++) {
        for (int x = -2; x <= 2; x++) {
            float v = LUMA_E_Q_tex(uv_b + vec2(float(x), float(y)) * LUMA_A_Q_pt).r;
            lo = min(lo, v);
            hi = max(hi, v);
        }
    }
...
vec4 hook() {
    ivec2 coord = ivec2(LUMA_B_Q_pos * LUMA_B_Q_size);
    if (!pair_changed)
        return imageLoad(FLOW_Q_ED_CACHE, coord);
...
//!TEXTURE FLOW_H_DE_CACHE
//!SIZE 1920 1080
//!FORMAT rgba32f
//!STORAGE

//!HOOK FRAME_MIX
//!BIND FLOW_H_DE_CACHE
//!BIND LUMA_A_H
//!BIND LUMA_B_H
//!BIND FLOW_Q_DE
//!BIND LUMA_D_H
...
//!HEIGHT HOOKED.h 2 /
//!COMPONENTS 2
//!DESC [sext] refine flow slot3->slot4 (half res)

// Same seed-snapping fix as the two coarser handoffs above -- here for
// the Q->H handoff: FLOW_Q_DE's own texel is 4 full-res px, so an
// unsnapped bilinear read would smear a real boundary across ~4px in
// every direction (~8px total) when seeding this level.
vec2 snap_texel(vec2 uv, vec2 size) {
    return (floor(uv * size) + 0.5) / size;
...
float sad3x3_h(vec2 uv_a, vec2 uv_b) {
    ...
    for (int y = -1; y <= 1; y++) {
        for (int x = -1; x <= 1; x++) {
            vec2 o = vec2(float(x), float(y)) * LUMA_A_H_pt;
            s += abs(LUMA_D_H_tex(uv_a + o).r - LUMA_E_H_tex(uv_b + o).r);
        }
    }
    return s;
}
...
float local_contrast_3x3_h(vec2 uv_a) {
    ...
    for (int y = -1; y <= 1; y++) {
        for (int x = -1; x <= 1; x++) {
            float v = LUMA_D_H_tex(uv_a + vec2(float(x), float(y)) * LUMA_A_H_pt).r;
            lo = min(lo, v);
            hi = max(hi, v);
        }
    }
...
vec4 hook() {
    ivec2 coord = ivec2(LUMA_A_H_pos * LUMA_A_H_size);
    if (!pair_changed)
        return imageLoad(FLOW_H_DE_CACHE, coord);
...
//!TEXTURE FLOW_H_ED_CACHE
//!SIZE 1920 1080
//!FORMAT rgba32f
//!STORAGE

//!HOOK FRAME_MIX
//!BIND FLOW_H_ED_CACHE
//!BIND LUMA_A_H
//!BIND LUMA_B_H
//!BIND FLOW_Q_ED
//!BIND LUMA_D_H
...
//!HEIGHT HOOKED.h 2 /
//!COMPONENTS 2
//!DESC [sext] refine flow slot4->slot3 (half res)
...
float sad3x3_h2_self(vec2 uv, vec2 o) {
    ...
    for (int y = -1; y <= 1; y++) {
        for (int x = -1; x <= 1; x++) {
            vec2 d = vec2(float(x), float(y)) * LUMA_A_H_pt;
            s += abs(LUMA_E_H_tex(uv + d).r - LUMA_E_H_tex(uv + o + d).r);
        }
    }
    return s;
}
float sad3x3_h2(vec2 uv_b, vec2 uv_a) {
    ...
    for (int y = -1; y <= 1; y++) {
        for (int x = -1; x <= 1; x++) {
            vec2 o = vec2(float(x), float(y)) * LUMA_A_H_pt;
            s += abs(LUMA_E_H_tex(uv_b + o).r - LUMA_D_H_tex(uv_a + o).r);
        }
    }
    return s;
}
...
float local_contrast_3x3_h2(vec2 uv_b) {
    ...
    for (int y = -1; y <= 1; y++) {
        for (int x = -1; x <= 1; x++) {
            float v = LUMA_E_H_tex(uv_b + vec2(float(x), float(y)) * LUMA_A_H_pt).r;
            lo = min(lo, v);
            hi = max(hi, v);
        }
    }
...
vec4 hook() {
    ivec2 coord = ivec2(LUMA_B_H_pos * LUMA_B_H_size);
    if (!pair_changed)
        return imageLoad(FLOW_H_ED_CACHE, coord);
...
//!BIND FLOW_H_DE
//!BIND FLOW_H_ED
//!BIND FLOW_H_ED_M1_ST
//!SAVE FLOW_H_DE
//!WIDTH HOOKED.w 2 /
//!HEIGHT HOOKED.h 2 /
//!COMPONENTS 2
//!DESC [sext] vector median filter on flow slot3->slot4 (pass 1) [fused with its B->A twin: one dispatch]
void hook_ba() {
    if (!pair_changed)
        { imageStore(FLOW_H_ED_M1_ST, ivec2(gl_FragCoord.xy), vec4(0.0)); return; }

    vec2 v[9];
    int n = 0;
    for (int y = -1; y <= 1; y++) {
        for (int x = -1; x <= 1; x++) {
            vec2 o = vec2(float(x), float(y)) * FLOW_H_ED_pt;
            v[n++] = FLOW_H_ED_tex(FLOW_H_ED_pos + o).xy;
        }
    }
    ...
    { imageStore(FLOW_H_ED_M1_ST, ivec2(gl_FragCoord.xy), vec4(best, 0.0, 0.0)); return; }
}
...
vec4 hook() {
    ...
    for (int y = -1; y <= 1; y++) {
        for (int x = -1; x <= 1; x++) {
            vec2 o = vec2(float(x), float(y)) * FLOW_H_DE_pt;
            v[n++] = FLOW_H_DE_tex(FLOW_H_DE_pos + o).xy;
        }
    }
...
// This one's result is what the final warp actually reads, so it gets a
// real persistent cache (unlike pass 1 above).
//!TEXTURE FLOW_H_DE_MEDIAN_CACHE
//!SIZE 1920 1080
//!FORMAT rgba32f
//!STORAGE

//!HOOK FRAME_MIX
//!BIND FLOW_H_DE_MEDIAN_CACHE
//!BIND FLOW_H_DE
//!SAVE FLOW_H_DE
//!WIDTH HOOKED.w 2 /
//!HEIGHT HOOKED.h 2 /
//!COMPONENTS 2
//!DESC [sext] vector median filter on flow slot3->slot4 (pass 2)
vec4 hook() {
    ivec2 coord = ivec2(FLOW_H_DE_pos * FLOW_H_DE_size);
    if (!pair_changed)
        return imageLoad(FLOW_H_DE_MEDIAN_CACHE, coord);
    ...
    vec4 result = vec4(best, 0.0, 0.0);
    imageStore(FLOW_H_DE_MEDIAN_CACHE, coord, result);
    return result;
}


//!TEXTURE FLOW_H_ED_MEDIAN_CACHE
//!SIZE 1920 1080
//!FORMAT rgba32f
//!STORAGE

//!HOOK FRAME_MIX
//!BIND FLOW_H_ED_MEDIAN_CACHE
//!BIND FLOW_H_ED_M1_ST
//!BIND FLOW_H_DE
//!SAVE FLOW_H_ED
//!WIDTH HOOKED.w 2 /
//!HEIGHT HOOKED.h 2 /
//!COMPONENTS 2
//!DESC [sext] vector median filter on flow slot4->slot3 (pass 2)
vec4 hook() {
    ivec2 coord = ivec2(FLOW_H_DE_pos * FLOW_H_DE_size);
    if (!pair_changed)
        return imageLoad(FLOW_H_ED_MEDIAN_CACHE, coord);
    ...
    for (int y = -1; y <= 1; y++) {
        for (int x = -1; x <= 1; x++) {
            vec2 o = vec2(float(x), float(y)) * FLOW_H_DE_pt;
            v[n++] = imageLoad(FLOW_H_ED_M1_ST, ivec2(floor((FLOW_H_DE_size) * (FLOW_H_DE_pos + o)))).xy;
        }
    }
    ...
    vec4 result = vec4(best, 0.0, 0.0);
    imageStore(FLOW_H_ED_MEDIAN_CACHE, coord, result);
    return result;
}
// =====================================================================
// SLOT 4 -> SLOT 5 flow chain ([sext], generated). The pair the fifth
// frame adds: the second link of the composed +2 displacement
// F24(x) = F23(x) + F34(x + F23(x)), the mirror of the quad's far flow.
// =====================================================================

// =====================================================================
// SLOT 5 -> SLOT 4 flow chain ([sext], generated). Reverse of the above,
// closing the round trip on the +2 displacement's second link.
// =====================================================================
...
// see the "Storage-based flow caching" note at the top of this file.
// ---------------------------------------------------------------------
//!TEXTURE FLOW_S_EF_CACHE
//!SIZE 240 135
//!FORMAT rgba32f
//!STORAGE

//!TEXTURE FLOW_S_EF_CACHE2
//!SIZE 240 135
//!FORMAT rgba32f
//!STORAGE

//!TEXTURE FLOW_S_FE_CACHE
//!SIZE 240 135
//!FORMAT rgba32f
//!STORAGE

//!TEXTURE FLOW_S_FE_CACHE2
//!SIZE 240 135
//!FORMAT rgba32f
//!STORAGE

//!HOOK FRAME_MIX
//!BIND FLOW_S_EF_CACHE
//!BIND FLOW_S_EF_CACHE2
//!BIND FLOW_S_FE_CACHE
//!BIND FLOW_S_FE_CACHE2
//!BIND LUMA_A_S
//!BIND LUMA_B_S
//!BIND LUMA_E_S
//!BIND LUMA_F_S
...
//!HEIGHT HOOKED.h 16 /
//!COMPONENTS 4
//!DESC [sext] coarse flow search slot4->slot5 and slot5->slot4 (1/16 res) [fused: one dispatch]
...
float sad5x5_s(vec2 uv_a, vec2 uv_b) {
    ...
    for (int y = -COARSE_WINDOW_RADIUS; y <= COARSE_WINDOW_RADIUS; y++) {
        for (int x = -COARSE_WINDOW_RADIUS; x <= COARSE_WINDOW_RADIUS; x++) {
            vec2 o = vec2(float(x), float(y)) * LUMA_A_S_pt;
            s += abs(LUMA_E_S_tex(uv_a + o).r - LUMA_F_S_tex(uv_b + o).r);
        }
    }
    return s;
}
...
float local_contrast_5x5_s(vec2 uv_a) {
    ...
    for (int y = -2; y <= 2; y++) {
        for (int x = -2; x <= 2; x++) {
            float v = LUMA_E_S_tex(uv_a + vec2(float(x), float(y)) * LUMA_A_S_pt).r;
            lo = min(lo, v);
            hi = max(hi, v);
        }
    }
...
float sad5x5_s2(vec2 uv_b, vec2 uv_a) {
    ...
    for (int y = -COARSE_WINDOW_RADIUS; y <= COARSE_WINDOW_RADIUS; y++) {
        for (int x = -COARSE_WINDOW_RADIUS; x <= COARSE_WINDOW_RADIUS; x++) {
            vec2 o = vec2(float(x), float(y)) * LUMA_A_S_pt;
            s += abs(LUMA_F_S_tex(uv_b + o).r - LUMA_E_S_tex(uv_a + o).r);
        }
    }
    return s;
}
float local_contrast_5x5_s2(vec2 uv_b) {
    ...
    for (int y = -2; y <= 2; y++) {
        for (int x = -2; x <= 2; x++) {
            float v = LUMA_F_S_tex(uv_b + vec2(float(x), float(y)) * LUMA_A_S_pt).r;
            lo = min(lo, v);
            hi = max(hi, v);
        }
    }
...
void coarse_ba() {
    ...
    vec2 uv_b = LUMA_A_S_pos;
    vec2 prev_s = imageLoad(FLOW_S_FE_CACHE, coord).xy * LUMA_A_S_pt;

    if (local_contrast_5x5_s2(uv_b) < MIN_CONTRAST) {
        vec4 result = vec4(0.0);
        imageStore(FLOW_S_FE_CACHE, coord, result);
        imageStore(FLOW_S_FE_CACHE2, coord, result);
        return;
    }
    ...
    vec2 off_b = descend_s2(uv_b, start_b, cost_b);
    vec2 off_c = descend_s2(uv_b, prev_s, cost_c);
    imageStore(FLOW_S_FE_CACHE2, coord, vec4(off_c / LUMA_A_S_pt, 0.0, 0.0));
    vec4 result = vec4(off_a / LUMA_A_S_pt, off_b / LUMA_A_S_pt);
    imageStore(FLOW_S_FE_CACHE, coord, result);
}
vec4 hook() {
    ivec2 coord = ivec2(LUMA_A_S_pos * LUMA_A_S_size);
    if (!pair_changed)
        return imageLoad(FLOW_S_EF_CACHE, coord);
    coarse_ba();

    vec2 uv_a = LUMA_A_S_pos;
    vec2 prev_s = imageLoad(FLOW_S_EF_CACHE, coord).xy * LUMA_A_S_pt;

    if (local_contrast_5x5_s(uv_a) < MIN_CONTRAST) {
        vec4 result = vec4(0.0);
        imageStore(FLOW_S_EF_CACHE, coord, result);
        imageStore(FLOW_S_EF_CACHE2, coord, result);
        return result;
    }
    ...
    vec2 off_b = descend_s(uv_a, start_b, cost_b);
    vec2 off_c = descend_s(uv_a, prev_s, cost_c);
    imageStore(FLOW_S_EF_CACHE2, coord, vec4(off_c / LUMA_A_S_pt, 0.0, 0.0));
    vec4 result = vec4(off_a / LUMA_A_S_pt, off_b / LUMA_A_S_pt);
    imageStore(FLOW_S_EF_CACHE, coord, result);
    return result;
}
...
//!TEXTURE FLOW_E_EF_CACHE
//!SIZE 480 270
//!FORMAT rgba32f
//!STORAGE

//!HOOK FRAME_MIX
//!BIND FLOW_E_EF_CACHE
//!BIND LUMA_A_E
//!BIND LUMA_B_E
//!BIND FLOW_S_EF
//!BIND FLOW_S_EF_CACHE2
//!BIND LUMA_A_S
//!BIND FLOW_E_FE_CACHE
//!BIND LUMA_B_S
//!BIND LUMA_E_S
//!BIND LUMA_F_S
//!BIND LUMA_E_E
...
//!HEIGHT HOOKED.h 8 /
//!COMPONENTS 2
//!DESC [sext] refine flow slot4->slot5 (1/8 res)

// Snaps to the exact center of whichever FLOW_S_EF texel this position
// falls in, before reading it as this level's search seed just below.
// An ordinary bilinear read here (as this used to be) blends between
// neighboring coarse-level vectors whenever the sample position isn't
// exactly on a coarse texel center -- which is most positions, since
...
// texel holds the object's true motion and its neighbor holds ~zero,
// that blend produces a smooth gradient of in-between seed vectors
// spanning roughly one full FLOW_S_EF texel width in every direction --
// 16 full-res pixels each way at this handoff specifically -- entirely
// independent of how correct the underlying FLOW_S_EF values are.
// Real-hardware testing already ruled out the coarse search's own
// matching-window size as the (sole) cause of the "nebulous cloud" seen
// bleeding from real motion boundaries into neighboring static content
// in interpolate-debug-overlay.glsl (see COARSE_WINDOW_RADIUS in the
...
float sad5x5_e(vec2 uv_a, vec2 uv_b) {
    ...
    for (int y = -COARSE_WINDOW_RADIUS; y <= COARSE_WINDOW_RADIUS; y++) {
        for (int x = -COARSE_WINDOW_RADIUS; x <= COARSE_WINDOW_RADIUS; x++) {
            vec2 o = vec2(float(x), float(y)) * LUMA_A_E_pt;
            s += abs(LUMA_E_E_tex(uv_a + o).r - LUMA_F_E_tex(uv_b + o).r);
        }
    }
    return s;
}
...
float local_contrast_5x5_e(vec2 uv_a) {
    ...
    for (int y = -2; y <= 2; y++) {
        for (int x = -2; x <= 2; x++) {
            float v = LUMA_E_E_tex(uv_a + vec2(float(x), float(y)) * LUMA_A_E_pt).r;
            lo = min(lo, v);
            hi = max(hi, v);
        }
    }
...
float frame_diff_s(vec2 uv) {
    ...
    for (int j = -1; j <= 1; j++) {
        for (int i = -1; i <= 1; i++) {
            vec2 c = uv + vec2(float(i), float(j)) * LUMA_A_S_pt;
            d = max(d, abs(LUMA_E_S_tex(c).r - LUMA_F_S_tex(c).r));
        }
    }
    return d;
}
...
float moire_s(vec2 uv) {
    ...
    for (int j = -1; j <= 1; j++) {
        for (int i = -1; i <= 1; i++) {
            vec2 c = uv + vec2(float(i), float(j)) * LUMA_A_S_pt;
            float p = LUMA_E_S_tex(c).r;
            float b = 0.25 * (LUMA_E_E_tex(c + vec2(-0.25, -0.25) * LUMA_A_S_pt).r + LUMA_E_E_tex(c + vec2(0.25, -0.25) * LUMA_A_S_pt).r
                            + LUMA_E_E_tex(c + vec2(-0.25, 0.25) * LUMA_A_S_pt).r + LUMA_E_E_tex(c + vec2(0.25, 0.25) * LUMA_A_S_pt).r);
            plo = min(plo, p); phi = max(phi, p); blo = min(blo, b); bhi = max(bhi, b);
        }
    }
    float cp = phi - plo, cb = bhi - blo;
...
vec4 hook() {
    ivec2 coord = ivec2(LUMA_A_E_pos * LUMA_A_E_size);
    if (!pair_changed)
        return imageLoad(FLOW_E_EF_CACHE, coord);
    ...
    // simply not be enough room for them to do that job.
    // ---- refine three seeds; the temporal seed and prior only where the previous flow round-trips ----
    vec2 prev_e = imageLoad(FLOW_E_EF_CACHE, coord).xy * LUMA_A_E_pt;
    ivec2 rcoord = clamp(ivec2((uv_a + prev_e) * LUMA_A_E_size), ivec2(0), ivec2(LUMA_A_E_size) - 1);
    vec2 prev_rev = imageLoad(FLOW_E_FE_CACHE, rcoord).xy * LUMA_A_E_pt;
    float rt = length((prev_e + prev_rev) / LUMA_A_E_pt);
    bool trusted = rt < SEED_RT_MAX && length(prev_e) > 0.0;
    float tl = trusted ? SEED_TEMP_LAMBDA : 0.0;
    ivec2 scoord = ivec2(snap_texel(uv_a, FLOW_S_EF_size) * FLOW_S_EF_size);
    vec2 base_off3 = imageLoad(FLOW_S_EF_CACHE2, scoord).xy * 2.0 * LUMA_A_E_pt;
    float sad_a, sad_b, sad_c;
    vec2 ref_a = refine_e(uv_a, base_off, sad_a);
    vec2 ref_b = refine_e(uv_a, base_off2, sad_b);
    vec2 ref_c = refine_e(uv_a, base_off3, sad_c);
...
//!HEIGHT HOOKED.h 8 /
//!COMPONENTS 2
//!DESC [sext] refine flow slot5->slot4 (1/8 res)
...
float sad5x5_e2(vec2 uv_b, vec2 uv_a) {
    ...
    for (int y = -COARSE_WINDOW_RADIUS; y <= COARSE_WINDOW_RADIUS; y++) {
        for (int x = -COARSE_WINDOW_RADIUS; x <= COARSE_WINDOW_RADIUS; x++) {
            vec2 o = vec2(float(x), float(y)) * LUMA_A_E_pt;
            s += abs(LUMA_F_E_tex(uv_b + o).r - LUMA_E_E_tex(uv_a + o).r);
        }
    }
    return s;
}
...
float local_contrast_5x5_e2(vec2 uv_b) {
    ...
    for (int y = -2; y <= 2; y++) {
        for (int x = -2; x <= 2; x++) {
            float v = LUMA_F_E_tex(uv_b + vec2(float(x), float(y)) * LUMA_A_E_pt).r;
            lo = min(lo, v);
            hi = max(hi, v);
        }
    }
...
float moire_s(vec2 uv) {
    ...
    for (int j = -1; j <= 1; j++) {
        for (int i = -1; i <= 1; i++) {
            vec2 c = uv + vec2(float(i), float(j)) * LUMA_B_S_pt;
            float p = LUMA_F_S_tex(c).r;
            float b = 0.25 * (LUMA_F_E_tex(c + vec2(-0.25, -0.25) * LUMA_B_S_pt).r + LUMA_F_E_tex(c + vec2(0.25, -0.25) * LUMA_B_S_pt).r
                            + LUMA_F_E_tex(c + vec2(-0.25, 0.25) * LUMA_B_S_pt).r + LUMA_F_E_tex(c + vec2(0.25, 0.25) * LUMA_B_S_pt).r);
            plo = min(plo, p); phi = max(phi, p); blo = min(blo, b); bhi = max(bhi, b);
        }
    }
    float cp = phi - plo, cb = bhi - blo;
...
bool aperture_ok(vec2 uv, vec2 off) {
    ...
    for (int j = -1; j <= 1; j++) {
        for (int i = -1; i <= 1; i++) {
            vec2 c = uv + vec2(float(i), float(j)) * LUMA_A_E_pt;
            float gx = LUMA_F_E_tex(c + vec2(LUMA_A_E_pt.x, 0.0)).r - LUMA_F_E_tex(c - vec2(LUMA_A_E_pt.x, 0.0)).r;
            float gy = LUMA_F_E_tex(c + vec2(0.0, LUMA_A_E_pt.y)).r - LUMA_F_E_tex(c - vec2(0.0, LUMA_A_E_pt.y)).r;
            jxx += gx * gx; jyy += gy * gy; jxy += gx * gy;
        }
    }
    float tr = jxx + jyy, det = jxx * jyy - jxy * jxy;
...
vec4 hook() {
    ivec2 coord = ivec2(LUMA_B_E_pos * LUMA_B_E_size);
    if (!pair_changed)
        return imageLoad(FLOW_E_FE_CACHE, coord);
    ...
    // See REFINE_SEARCH_RADIUS/REFINE_REG_LAMBDA in the A->B pass above.
    // ---- refine three seeds; the temporal seed and prior only where the previous flow round-trips ----
    vec2 prev_e = imageLoad(FLOW_E_FE_CACHE, coord).xy * LUMA_A_E_pt;
    ivec2 rcoord = clamp(ivec2((uv_b + prev_e) * LUMA_A_E_size), ivec2(0), ivec2(LUMA_A_E_size) - 1);
    vec2 prev_rev = imageLoad(FLOW_E_EF_CACHE, rcoord).xy * LUMA_A_E_pt;
    float rt = length((prev_e + prev_rev) / LUMA_A_E_pt);
    bool trusted = rt < SEED_RT_MAX && length(prev_e) > 0.0;
    float tl = trusted ? SEED_TEMP_LAMBDA : 0.0;
    ivec2 scoord = ivec2(snap_texel(uv_b, LUMA_B_S_size) * LUMA_B_S_size);
    vec2 base_off3 = imageLoad(FLOW_S_FE_CACHE2, scoord).xy * 2.0 * LUMA_A_E_pt;
    float sad_a, sad_b, sad_c;
    vec2 ref_a = refine_e(uv_b, base_off, sad_a);
    vec2 ref_b = refine_e(uv_b, base_off2, sad_b);
    vec2 ref_c = refine_e(uv_b, base_off3, sad_c);
...
//!HEIGHT HOOKED.h 8 /
//!COMPONENTS 2
//!DESC [prop12] contrast-weighted flow propagation slot4->slot5 (pass 1 of 3) [fused with its B->A twin: one dispatch]
...
float prop_conf(vec2 uv) {
    ...
    for (int y = -2; y <= 2; y++)
        for (int x = -2; x <= 2; x++) {
            float l = LUMA_E_E_tex(uv + vec2(float(x), float(y)) * LUMA_A_E_pt).r;
            lo = min(lo, l); hi = max(hi, l);
        }
    return clamp((hi - lo) / PROP_CONF_FULL, 0.0, 1.0);
}
...
void hook_ba() {
    vec2 uv = FLOW_E_FE_RAW_pos;
    vec2 own = FLOW_E_FE_RAW_tex(uv).xy;
    float c_own = prop_conf(uv);
    vec2 acc = vec2(0.0);
    float wsum = 0.0;
    for (int y = -2; y <= 2; y++)
        for (int x = -2; x <= 2; x++) {
            if (x == 0 && y == 0) continue;
            vec2 o = vec2(float(x), float(y)) * FLOW_E_FE_RAW_pt;
            float w = prop_conf(uv + o) / (1.0 + 0.5 * float(abs(x) + abs(y)));
            acc += w * FLOW_E_FE_RAW_tex(uv + o).xy;
            wsum += w;
        }
    float w_own = PROP_SELF_WEIGHT * c_own * c_own;
    if (wsum + w_own <= 0.0)
        { imageStore(FLOW_E_FE_PROP_ST1, ivec2(gl_FragCoord.xy), vec4(own, 0.0, 0.0)); return; }
    { imageStore(FLOW_E_FE_PROP_ST1, ivec2(gl_FragCoord.xy), vec4((w_own * own + acc) / (w_own + wsum), 0.0, 0.0)); return; }
}
vec4 hook() {
    hook_ba();
    vec2 uv = FLOW_E_EF_RAW_pos;
    vec2 own = FLOW_E_EF_RAW_tex(uv).xy;
    float c_own = prop_conf(uv);
    vec2 acc = vec2(0.0);
    float wsum = 0.0;
    for (int y = -2; y <= 2; y++)
        for (int x = -2; x <= 2; x++) {
            if (x == 0 && y == 0) continue;
            vec2 o = vec2(float(x), float(y)) * FLOW_E_EF_RAW_pt;
            float w = prop_conf(uv + o) / (1.0 + 0.5 * float(abs(x) + abs(y)));
            acc += w * FLOW_E_EF_RAW_tex(uv + o).xy;
            wsum += w;
        }
    float w_own = PROP_SELF_WEIGHT * c_own * c_own;
    if (wsum + w_own <= 0.0)
...
//!HEIGHT HOOKED.h 8 /
//!COMPONENTS 2
//!DESC [prop12] contrast-weighted flow propagation slot4->slot5 (pass 2 of 3) [fused with its B->A twin: one dispatch]
...
void hook_ba() {
    vec2 uv = FLOW_E_EF_PROP_pos;
    vec2 own = imageLoad(FLOW_E_FE_PROP_ST1, ivec2(floor((FLOW_E_EF_PROP_size) * (uv)))).xy;
    float c_own = prop_conf(uv);
    vec2 acc = vec2(0.0);
    float wsum = 0.0;
    for (int y = -2; y <= 2; y++)
        for (int x = -2; x <= 2; x++) {
            if (x == 0 && y == 0) continue;
            vec2 o = vec2(float(x), float(y)) * FLOW_E_EF_PROP_pt;
            float w = prop_conf(uv + o) / (1.0 + 0.5 * float(abs(x) + abs(y)));
            acc += w * imageLoad(FLOW_E_FE_PROP_ST1, ivec2(floor((FLOW_E_EF_PROP_size) * (uv + o)))).xy;
            wsum += w;
        }
    float w_own = PROP_SELF_WEIGHT * c_own * c_own;
    if (wsum + w_own <= 0.0)
        { imageStore(FLOW_E_FE_PROP_ST2, ivec2(gl_FragCoord.xy), vec4(own, 0.0, 0.0)); return; }
    { imageStore(FLOW_E_FE_PROP_ST2, ivec2(gl_FragCoord.xy), vec4((w_own * own + acc) / (w_own + wsum), 0.0, 0.0)); return; }
}
vec4 hook() {
    hook_ba();
    vec2 uv = FLOW_E_EF_PROP_pos;
    vec2 own = FLOW_E_EF_PROP_tex(uv).xy;
    float c_own = prop_conf(uv);
    vec2 acc = vec2(0.0);
    float wsum = 0.0;
    for (int y = -2; y <= 2; y++)
        for (int x = -2; x <= 2; x++) {
            ...
            vec2 o = vec2(float(x), float(y)) * FLOW_E_EF_PROP_pt;
            float w = prop_conf(uv + o) / (1.0 + 0.5 * float(abs(x) + abs(y)));
            acc += w * FLOW_E_EF_PROP_tex(uv + o).xy;
            wsum += w;
        }
    float w_own = PROP_SELF_WEIGHT * c_own * c_own;
    if (wsum + w_own <= 0.0)
...
//!HEIGHT HOOKED.h 8 /
//!COMPONENTS 2
//!DESC [prop12] contrast-weighted flow propagation slot4->slot5 (pass 3 of 3) [fused with its B->A twin: one dispatch]
...
void hook_ba() {
    vec2 uv = FLOW_E_EF_PROP_pos;
    vec2 own = imageLoad(FLOW_E_FE_PROP_ST2, ivec2(floor((FLOW_E_EF_PROP_size) * (uv)))).xy;
    float c_own = prop_conf(uv);
    vec2 acc = vec2(0.0);
    float wsum = 0.0;
    for (int y = -2; y <= 2; y++)
        for (int x = -2; x <= 2; x++) {
            ...
            vec2 o = vec2(float(x), float(y)) * FLOW_E_EF_PROP_pt;
            float w = prop_conf(uv + o) / (1.0 + 0.5 * float(abs(x) + abs(y)));
            acc += w * imageLoad(FLOW_E_FE_PROP_ST2, ivec2(floor((FLOW_E_EF_PROP_size) * (uv + o)))).xy;
            wsum += w;
        }
    float w_own = PROP_SELF_WEIGHT * c_own * c_own;
    if (wsum + w_own <= 0.0)
        { imageStore(FLOW_E_FE_PROP_ST3, ivec2(gl_FragCoord.xy), vec4(own, 0.0, 0.0)); return; }
    { imageStore(FLOW_E_FE_PROP_ST3, ivec2(gl_FragCoord.xy), vec4((w_own * own + acc) / (w_own + wsum), 0.0, 0.0)); return; }
}
vec4 hook() {
    hook_ba();
    vec2 uv = FLOW_E_EF_PROP_pos;
...
//!HEIGHT HOOKED.h 8 /
//!COMPONENTS 2
//!DESC [prop12] three-way data check slot4->slot5: propagated vs raw vs zero, ties to consensus on texture only [fused with its B->A twin: one dispatch]
...
float sad5(vec2 uv, vec2 flow_uv) {
    ...
    for (int y = -2; y <= 2; y++)
        for (int x = -2; x <= 2; x++) {
            vec2 o = vec2(float(x), float(y)) * LUMA_A_E_pt;
            s += abs(LUMA_E_E_tex(uv + o).r - LUMA_F_E_tex(uv + o + flow_uv).r);
        }
    return s;
}
...
float prop_conf_ba(vec2 uv) {
    ...
    for (int y = -2; y <= 2; y++)
        for (int x = -2; x <= 2; x++) {
            float l = LUMA_F_E_tex(uv + vec2(float(x), float(y)) * LUMA_B_E_pt).r;
            lo = min(lo, l); hi = max(hi, l);
        }
    return clamp((hi - lo) / PROP_CONF_FULL, 0.0, 1.0);
}
...
float sad5_ba(vec2 uv, vec2 flow_uv) {
    ...
    for (int y = -2; y <= 2; y++)
        for (int x = -2; x <= 2; x++) {
            vec2 o = vec2(float(x), float(y)) * LUMA_B_E_pt;
            s += abs(LUMA_F_E_tex(uv + o).r - LUMA_E_E_tex(uv + o + flow_uv).r);
        }
    return s;
}
...
void hook_ba() {
    vec2 uv = FLOW_E_EF_PROP_pos;
    vec2 raw = FLOW_E_FE_RAW_tex(uv).xy;
    vec2 prop = imageLoad(FLOW_E_FE_PROP_ST3, ivec2(floor((FLOW_E_EF_PROP_size) * (uv)))).xy;
    if (prop == raw)
        { imageStore(FLOW_E_FE_ST, ivec2(gl_FragCoord.xy), vec4(raw, 0.0, 0.0)); return; }
    float s_prop = sad5_ba(uv, prop * LUMA_B_E_pt);
    float s_raw  = sad5_ba(uv, raw * LUMA_B_E_pt);
    float s_zero = sad5_ba(uv, vec2(0.0));
    float best = min(s_raw, s_zero);
    ...
    if (textured) {
        ...
        // the three-way rule as before.
        if (s_prop < best * (1.0 - PROP_CHECK_MARGIN) - 1e-4)
            { imageStore(FLOW_E_FE_ST, ivec2(gl_FragCoord.xy), vec4(prop, 0.0, 0.0)); return; }
        // prop12: the disagreement threshold scales with the flow, so a fast
        // translation whose consensus is diluted by background votes near its
        // edges is not mistaken for an alias (aliases sit texels apart at
        // speeds of a texel or two).
        if (length(prop - raw) > max(PROP_DISAGREE, PROP_DISAGREE_REL * length(raw)))
            { imageStore(FLOW_E_FE_ST, ivec2(gl_FragCoord.xy), vec4(0.0)); return; }
        if (s_prop <= best * (1.0 + PROP_CHECK_MARGIN) + 1e-4)
            { imageStore(FLOW_E_FE_ST, ivec2(gl_FragCoord.xy), vec4(prop, 0.0, 0.0)); return; }
        { imageStore(FLOW_E_FE_ST, ivec2(gl_FragCoord.xy), s_raw <= s_zero ? vec4(raw, 0.0, 0.0) : vec4(0.0)); return; }
    }
    // flat: evidence required
    if (s_prop < best * (1.0 - PROP_CHECK_MARGIN) - 1e-4)
        { imageStore(FLOW_E_FE_ST, ivec2(gl_FragCoord.xy), vec4(prop, 0.0, 0.0)); return; }
...
vec4 hook() {
    hook_ba();
    vec2 uv = FLOW_E_EF_PROP_pos;
    vec2 raw = FLOW_E_EF_RAW_tex(uv).xy;
    vec2 prop = FLOW_E_EF_PROP_tex(uv).xy;
    if (prop == raw)
        return vec4(raw, 0.0, 0.0);
    float s_prop = sad5(uv, prop * LUMA_A_E_pt);
    float s_raw  = sad5(uv, raw * LUMA_A_E_pt);
...
//!TEXTURE FLOW_Q_EF_CACHE
//!SIZE 960 540
//!FORMAT rgba32f
//!STORAGE

//!HOOK FRAME_MIX
//!BIND FLOW_Q_EF_CACHE
//!BIND LUMA_A_Q
//!BIND LUMA_B_Q
//!BIND FLOW_E_EF
//!BIND LUMA_E_Q
...
//!HEIGHT HOOKED.h 4 /
//!COMPONENTS 2
//!DESC [sext] refine flow slot4->slot5 (1/4 res)

// Same seed-snapping fix as the S->E handoff above (see that pass for
// the full reasoning) -- here for the E->Q handoff: FLOW_E_EF's own
// texel is 8 full-res px, so an unsnapped bilinear read would smear a
// real boundary across ~8px in every direction (~16px total) when
// seeding this level, on top of whatever the S->E handoff already did.
vec2 snap_texel(vec2 uv, vec2 size) {
...
float sad5x5_q(vec2 uv_a, vec2 uv_b) {
    ...
    for (int y = -COARSE_WINDOW_RADIUS; y <= COARSE_WINDOW_RADIUS; y++) {
        for (int x = -COARSE_WINDOW_RADIUS; x <= COARSE_WINDOW_RADIUS; x++) {
            vec2 o = vec2(float(x), float(y)) * LUMA_A_Q_pt;
            s += abs(LUMA_E_Q_tex(uv_a + o).r - LUMA_F_Q_tex(uv_b + o).r);
        }
    }
    return s;
}
...
float local_contrast_5x5_q(vec2 uv_a) {
    ...
    for (int y = -2; y <= 2; y++) {
        for (int x = -2; x <= 2; x++) {
            float v = LUMA_E_Q_tex(uv_a + vec2(float(x), float(y)) * LUMA_A_Q_pt).r;
            lo = min(lo, v);
            hi = max(hi, v);
        }
    }
...
vec4 hook() {
    ivec2 coord = ivec2(LUMA_A_Q_pos * LUMA_A_Q_size);
    if (!pair_changed)
        return imageLoad(FLOW_Q_EF_CACHE, coord);
...
//!TEXTURE FLOW_Q_FE_CACHE
//!SIZE 960 540
//!FORMAT rgba32f
//!STORAGE

//!HOOK FRAME_MIX
//!BIND FLOW_Q_FE_CACHE
//!BIND LUMA_A_Q
//!BIND LUMA_B_Q
//!BIND FLOW_E_FE_ST
//!BIND FLOW_E_EF
...
//!HEIGHT HOOKED.h 4 /
//!COMPONENTS 2
//!DESC [sext] refine flow slot5->slot4 (1/4 res)
...
float sad5x5_q2(vec2 uv_b, vec2 uv_a) {
    ...
    for (int y = -COARSE_WINDOW_RADIUS; y <= COARSE_WINDOW_RADIUS; y++) {
        for (int x = -COARSE_WINDOW_RADIUS; x <= COARSE_WINDOW_RADIUS; x++) {
            vec2 o = vec2(float(x), float(y)) * LUMA_A_Q_pt;
            s += abs(LUMA_F_Q_tex(uv_b + o).r - LUMA_E_Q_tex(uv_a + o).r);
        }
    }
    return s;
}
...
float local_contrast_5x5_q2(vec2 uv_b) {
    ...
    for (int y = -2; y <= 2; y++) {
        for (int x = -2; x <= 2; x++) {
            float v = LUMA_F_Q_tex(uv_b + vec2(float(x), float(y)) * LUMA_A_Q_pt).r;
            lo = min(lo, v);
            hi = max(hi, v);
        }
    }
...
vec4 hook() {
    ivec2 coord = ivec2(LUMA_B_Q_pos * LUMA_B_Q_size);
    if (!pair_changed)
        return imageLoad(FLOW_Q_FE_CACHE, coord);
...
//!TEXTURE FLOW_H_EF_CACHE
//!SIZE 1920 1080
//!FORMAT rgba32f
//!STORAGE

//!HOOK FRAME_MIX
//!BIND FLOW_H_EF_CACHE
//!BIND LUMA_A_H
//!BIND LUMA_B_H
//!BIND FLOW_Q_EF
//!BIND LUMA_E_H
...
//!HEIGHT HOOKED.h 2 /
//!COMPONENTS 2
//!DESC [sext] refine flow slot4->slot5 (half res)

// Same seed-snapping fix as the two coarser handoffs above -- here for
// the Q->H handoff: FLOW_Q_EF's own texel is 4 full-res px, so an
// unsnapped bilinear read would smear a real boundary across ~4px in
// every direction (~8px total) when seeding this level.
vec2 snap_texel(vec2 uv, vec2 size) {
    return (floor(uv * size) + 0.5) / size;
...
float sad3x3_h(vec2 uv_a, vec2 uv_b) {
    ...
    for (int y = -1; y <= 1; y++) {
        for (int x = -1; x <= 1; x++) {
            vec2 o = vec2(float(x), float(y)) * LUMA_A_H_pt;
            s += abs(LUMA_E_H_tex(uv_a + o).r - LUMA_F_H_tex(uv_b + o).r);
        }
    }
    return s;
}
...
float local_contrast_3x3_h(vec2 uv_a) {
    ...
    for (int y = -1; y <= 1; y++) {
        for (int x = -1; x <= 1; x++) {
            float v = LUMA_E_H_tex(uv_a + vec2(float(x), float(y)) * LUMA_A_H_pt).r;
            lo = min(lo, v);
            hi = max(hi, v);
        }
    }
...
vec4 hook() {
    ivec2 coord = ivec2(LUMA_A_H_pos * LUMA_A_H_size);
    if (!pair_changed)
        return imageLoad(FLOW_H_EF_CACHE, coord);
...
//!TEXTURE FLOW_H_FE_CACHE
//!SIZE 1920 1080
//!FORMAT rgba32f
//!STORAGE

//!HOOK FRAME_MIX
//!BIND FLOW_H_FE_CACHE
//!BIND LUMA_A_H
//!BIND LUMA_B_H
//!BIND FLOW_Q_FE
//!BIND LUMA_E_H
...
//!HEIGHT HOOKED.h 2 /
//!COMPONENTS 2
//!DESC [sext] refine flow slot5->slot4 (half res)
...
float sad3x3_h2(vec2 uv_b, vec2 uv_a) {
    ...
    for (int y = -1; y <= 1; y++) {
        for (int x = -1; x <= 1; x++) {
            vec2 o = vec2(float(x), float(y)) * LUMA_A_H_pt;
            s += abs(LUMA_F_H_tex(uv_b + o).r - LUMA_E_H_tex(uv_a + o).r);
        }
    }
    return s;
}
...
float local_contrast_3x3_h2(vec2 uv_b) {
    ...
    for (int y = -1; y <= 1; y++) {
        for (int x = -1; x <= 1; x++) {
            float v = LUMA_F_H_tex(uv_b + vec2(float(x), float(y)) * LUMA_A_H_pt).r;
            lo = min(lo, v);
            hi = max(hi, v);
        }
    }
...
vec4 hook() {
    ivec2 coord = ivec2(LUMA_B_H_pos * LUMA_B_H_size);
    if (!pair_changed)
        return imageLoad(FLOW_H_FE_CACHE, coord);
...
//!BIND FLOW_H_EF
//!BIND FLOW_H_FE
//!BIND FLOW_H_FE_M1_ST
//!SAVE FLOW_H_EF
//!WIDTH HOOKED.w 2 /
//!HEIGHT HOOKED.h 2 /
//!COMPONENTS 2
//!DESC [sext] vector median filter on flow slot4->slot5 (pass 1) [fused with its B->A twin: one dispatch]
void hook_ba() {
    if (!pair_changed)
        { imageStore(FLOW_H_FE_M1_ST, ivec2(gl_FragCoord.xy), vec4(0.0)); return; }

    vec2 v[9];
    int n = 0;
    for (int y = -1; y <= 1; y++) {
        for (int x = -1; x <= 1; x++) {
            vec2 o = vec2(float(x), float(y)) * FLOW_H_FE_pt;
            v[n++] = FLOW_H_FE_tex(FLOW_H_FE_pos + o).xy;
        }
    }
    ...
    { imageStore(FLOW_H_FE_M1_ST, ivec2(gl_FragCoord.xy), vec4(best, 0.0, 0.0)); return; }
}
...
vec4 hook() {
    ...
    for (int y = -1; y <= 1; y++) {
        for (int x = -1; x <= 1; x++) {
            vec2 o = vec2(float(x), float(y)) * FLOW_H_EF_pt;
            v[n++] = FLOW_H_EF_tex(FLOW_H_EF_pos + o).xy;
        }
    }
...
// This one's result is what the final warp actually reads, so it gets a
// real persistent cache (unlike pass 1 above).
//!TEXTURE FLOW_H_EF_MEDIAN_CACHE
//!SIZE 1920 1080
//!FORMAT rgba32f
//!STORAGE

//!HOOK FRAME_MIX
//!BIND FLOW_H_EF_MEDIAN_CACHE
//!BIND FLOW_H_EF
//!SAVE FLOW_H_EF
//!WIDTH HOOKED.w 2 /
//!HEIGHT HOOKED.h 2 /
//!COMPONENTS 2
//!DESC [sext] vector median filter on flow slot4->slot5 (pass 2)
vec4 hook() {
    ivec2 coord = ivec2(FLOW_H_EF_pos * FLOW_H_EF_size);
    if (!pair_changed)
        return imageLoad(FLOW_H_EF_MEDIAN_CACHE, coord);
    ...
    vec4 result = vec4(best, 0.0, 0.0);
    imageStore(FLOW_H_EF_MEDIAN_CACHE, coord, result);
    return result;
}


//!TEXTURE FLOW_H_FE_MEDIAN_CACHE
//!SIZE 1920 1080
//!FORMAT rgba32f
//!STORAGE

//!HOOK FRAME_MIX
//!BIND FLOW_H_FE_MEDIAN_CACHE
//!BIND FLOW_H_FE_M1_ST
//!BIND FLOW_H_EF
//!SAVE FLOW_H_FE
//!WIDTH HOOKED.w 2 /
//!HEIGHT HOOKED.h 2 /
//!COMPONENTS 2
//!DESC [sext] vector median filter on flow slot5->slot4 (pass 2)
vec4 hook() {
    ivec2 coord = ivec2(FLOW_H_EF_pos * FLOW_H_EF_size);
    if (!pair_changed)
        return imageLoad(FLOW_H_FE_MEDIAN_CACHE, coord);
    ...
    for (int y = -1; y <= 1; y++) {
        for (int x = -1; x <= 1; x++) {
            vec2 o = vec2(float(x), float(y)) * FLOW_H_EF_pt;
            v[n++] = imageLoad(FLOW_H_FE_M1_ST, ivec2(floor((FLOW_H_EF_size) * (FLOW_H_EF_pos + o)))).xy;
        }
    }
    ...
    vec4 result = vec4(best, 0.0, 0.0);
    imageStore(FLOW_H_FE_MEDIAN_CACHE, coord, result);
    return result;
}
//!HOOK FRAME_MIX
//!BIND HOOKED
...
//!HEIGHT HOOKED.h
//!COMPONENTS 1
//!DESC [sext] slot 0 luma at full res
vec4 hook() {
    return vec4(dot(HOOKED_tex(HOOKED_pos).rgb, vec3(0.299, 0.587, 0.114)), 0, 0, 0);
}
//!HOOK FRAME_MIX
...
//!HEIGHT HOOKED.h
//!COMPONENTS 1
//!DESC [sext] slot 1 luma at full res
vec4 hook() {
    return vec4(dot(FRAME1_tex(HOOKED_pos).rgb, vec3(0.299, 0.587, 0.114)), 0, 0, 0);
}
//!HOOK FRAME_MIX
...
//!HEIGHT HOOKED.h
//!COMPONENTS 1
//!DESC [sext] slot 2 luma at full res
vec4 hook() {
    return vec4(dot(FRAME2_tex(HOOKED_pos).rgb, vec3(0.299, 0.587, 0.114)), 0, 0, 0);
}
//!HOOK FRAME_MIX
...
//!HEIGHT HOOKED.h
//!COMPONENTS 1
//!DESC [sext] slot 3 luma at full res
vec4 hook() {
    return vec4(dot(FRAME3_tex(HOOKED_pos).rgb, vec3(0.299, 0.587, 0.114)), 0, 0, 0);
}
//!HOOK FRAME_MIX
...
//!HEIGHT HOOKED.h
//!COMPONENTS 1
//!DESC [sext] slot 4 luma at full res
vec4 hook() {
    return vec4(dot(FRAME4_tex(HOOKED_pos).rgb, vec3(0.299, 0.587, 0.114)), 0, 0, 0);
}
//!HOOK FRAME_MIX
...
//!HEIGHT HOOKED.h
//!COMPONENTS 1
//!DESC [sext] slot 5 luma at full res
vec4 hook() {
    return vec4(dot(FRAME5_tex(HOOKED_pos).rgb, vec3(0.299, 0.587, 0.114)), 0, 0, 0);
}
...
//!HEIGHT HOOKED.h
//!COMPONENTS 2
//!DESC [sext] refine flow A->B (full res)
...
//!HEIGHT HOOKED.h
//!COMPONENTS 2
//!DESC [sext] refine flow B->A (full res)
...
//!HEIGHT HOOKED.h
//!COMPONENTS 2
//!DESC [sext] refine flow slot1->slot2 (full res)
...
//!HEIGHT HOOKED.h
//!COMPONENTS 2
//!DESC [sext] refine flow slot2->slot1 (full res)
...
//!HEIGHT HOOKED.h
//!COMPONENTS 2
//!DESC [sext] refine flow slot2->slot3 (full res)
...
//!HEIGHT HOOKED.h
//!COMPONENTS 2
//!DESC [sext] refine flow slot3->slot2 (full res)
...
//!TEXTURE FLOW_F_DE_CACHE
//!SIZE 1920 1080
//!FORMAT rgba32f
//!STORAGE

//!HOOK FRAME_MIX
//!BIND FLOW_F_DE_CACHE
//!BIND LUMA_A_F
//!BIND LUMA_B_F
//!BIND FLOW_H_DE
//!BIND LUMA_D_F
...
//!HEIGHT HOOKED.h
//!COMPONENTS 2
//!DESC [sext] refine flow slot3->slot4 (full res)

// Same seed-snapping fix as the two coarser handoffs above -- here for
// the H->F handoff: FLOW_H_DE's own texel is 2 full-res px, so an
// unsnapped bilinear read would smear a real boundary across ~2px in
// every direction (~4px total) when seeding this level.
vec2 snap_texel(vec2 uv, vec2 size) {
    return (floor(uv * size) + 0.5) / size;
...
float sad5x5_f(vec2 uv_a, vec2 uv_b) {
    ...
    for (int y = -2; y <= 2; y++) {
        for (int x = -2; x <= 2; x++) {
            vec2 o = vec2(float(x), float(y)) * LUMA_A_F_pt;
            s += abs(LUMA_D_F_tex(uv_a + o).r - LUMA_E_F_tex(uv_b + o).r);
        }
    }
    return s * (9.0 / 25.0);
}
...
float local_contrast_3x3_f(vec2 uv_a) {
    ...
    for (int y = -1; y <= 1; y++) {
        for (int x = -1; x <= 1; x++) {
            float v = LUMA_D_F_tex(uv_a + vec2(float(x), float(y)) * LUMA_A_F_pt).r;
            lo = min(lo, v);
            hi = max(hi, v);
        }
    }
...
vec4 hook() {
    ivec2 coord = ivec2(LUMA_A_F_pos * LUMA_A_F_size);
    if (!pair_changed)
        return imageLoad(FLOW_F_DE_CACHE, coord);
...
//!TEXTURE FLOW_F_ED_CACHE
//!SIZE 1920 1080
//!FORMAT rgba32f
//!STORAGE

//!HOOK FRAME_MIX
//!BIND FLOW_F_ED_CACHE
//!BIND LUMA_A_F
//!BIND LUMA_B_F
//!BIND FLOW_H_ED
//!BIND LUMA_D_F
...
//!HEIGHT HOOKED.h
//!COMPONENTS 2
//!DESC [sext] refine flow slot4->slot3 (full res)
...
float sad5x5_f2_self(vec2 uv, vec2 o) {
    ...
    for (int y = -1; y <= 1; y++) {
        for (int x = -1; x <= 1; x++) {
            vec2 d = vec2(float(x), float(y)) * LUMA_A_F_pt;
            s += abs(LUMA_E_F_tex(uv + d).r - LUMA_E_F_tex(uv + o + d).r);
        }
    }
    return s;
}
float sad5x5_f2(vec2 uv_b, vec2 uv_a) {
    ...
    for (int y = -2; y <= 2; y++) {
        for (int x = -2; x <= 2; x++) {
            vec2 o = vec2(float(x), float(y)) * LUMA_A_F_pt;
            s += abs(LUMA_E_F_tex(uv_b + o).r - LUMA_D_F_tex(uv_a + o).r);
        }
    }
    return s * (9.0 / 25.0);
}
...
float local_contrast_3x3_f2(vec2 uv_b) {
    ...
    for (int y = -1; y <= 1; y++) {
        for (int x = -1; x <= 1; x++) {
            float v = LUMA_E_F_tex(uv_b + vec2(float(x), float(y)) * LUMA_A_F_pt).r;
            lo = min(lo, v);
            hi = max(hi, v);
        }
    }
...
vec4 hook() {
    ivec2 coord = ivec2(LUMA_B_F_pos * LUMA_B_F_size);
    if (!pair_changed)
        return imageLoad(FLOW_F_ED_CACHE, coord);
...
//!TEXTURE FLOW_F_EF_CACHE
//!SIZE 1920 1080
//!FORMAT rgba32f
//!STORAGE

//!HOOK FRAME_MIX
//!BIND FLOW_F_EF_CACHE
//!BIND LUMA_A_F
//!BIND LUMA_B_F
//!BIND FLOW_H_EF
//!BIND LUMA_E_F
...
//!HEIGHT HOOKED.h
//!COMPONENTS 2
//!DESC [sext] refine flow slot4->slot5 (full res)

// Same seed-snapping fix as the two coarser handoffs above -- here for
// the H->F handoff: FLOW_H_EF's own texel is 2 full-res px, so an
// unsnapped bilinear read would smear a real boundary across ~2px in
// every direction (~4px total) when seeding this level.
vec2 snap_texel(vec2 uv, vec2 size) {
    return (floor(uv * size) + 0.5) / size;
...
float local_contrast_3x3_f(vec2 uv_a) {
    ...
    for (int y = -1; y <= 1; y++) {
        for (int x = -1; x <= 1; x++) {
            float v = LUMA_E_F_tex(uv_a + vec2(float(x), float(y)) * LUMA_A_F_pt).r;
            lo = min(lo, v);
            hi = max(hi, v);
        }
    }
...
vec4 hook() {
    ivec2 coord = ivec2(LUMA_A_F_pos * LUMA_A_F_size);
    if (!pair_changed)
        return imageLoad(FLOW_F_EF_CACHE, coord);
...
//!TEXTURE FLOW_F_FE_CACHE
//!SIZE 1920 1080
//!FORMAT rgba32f
//!STORAGE

//!HOOK FRAME_MIX
//!BIND FLOW_F_FE_CACHE
//!BIND LUMA_A_F
//!BIND LUMA_B_F
//!BIND FLOW_H_FE
//!BIND LUMA_E_F
...
//!HEIGHT HOOKED.h
//!COMPONENTS 2
//!DESC [sext] refine flow slot5->slot4 (full res)
...
vec4 hook() {
    ivec2 coord = ivec2(LUMA_B_F_pos * LUMA_B_F_size);
    if (!pair_changed)
        return imageLoad(FLOW_F_FE_CACHE, coord);
...
// ---------------------------------------------------------------------
// PACKING for the bind ceiling ([sext], generated). libplacebo binds at
...
//!WIDTH HOOKED.w 2 /
//!HEIGHT HOOKED.h 2 /
//!DESC [sext] pack the half-res straddle flows 0->1 (rg) and 1->2 (ba)
vec4 hook() {
...
//!WIDTH HOOKED.w 2 /
//!HEIGHT HOOKED.h 2 /
//!DESC [sext] pack the half-res straddle flows 2->3 (rg) and 3->4 (ba)
vec4 hook() {
...
//!WIDTH HOOKED.w
//!HEIGHT HOOKED.h
//!DESC [sext] pack the full-res flows slot0->slot1 (rg) and slot1->slot0 (ba)
vec4 hook() {
    return vec4(FLOW_F_AB_tex(FLOW_F_AB_pos).xy, FLOW_F_BA_tex(FLOW_F_BA_pos).xy);
}
...
//!WIDTH HOOKED.w
//!HEIGHT HOOKED.h
//!DESC [sext] pack the full-res flows slot1->slot2 (rg) and slot2->slot1 (ba)
vec4 hook() {
    return vec4(FLOW_F_BC_tex(FLOW_F_BC_pos).xy, FLOW_F_CB_tex(FLOW_F_CB_pos).xy);
}
...
//!WIDTH HOOKED.w
//!HEIGHT HOOKED.h
//!DESC [sext] pack the full-res flows slot2->slot3 (rg) and slot3->slot2 (ba)
vec4 hook() {
    return vec4(FLOW_F_CD_tex(FLOW_F_CD_pos).xy, FLOW_F_DC_tex(FLOW_F_DC_pos).xy);
}
...
//!WIDTH HOOKED.w
//!HEIGHT HOOKED.h
//!DESC [sext] pack the full-res flows slot3->slot4 (rg) and slot4->slot3 (ba)
vec4 hook() {
...
//!WIDTH HOOKED.w
//!HEIGHT HOOKED.h
//!DESC [sext] motion-compensated warp (cubic placement) and the least-squares field
...
//   6 = the fit's residual (px)  7 = velocity, the straddle flow
...
vec2 flow_h(int k, vec2 uv) {
    vec4 t = (k < 2) ? FLOW_HP0_tex(uv) : (k < 4) ? FLOW_HP1_tex(uv) : FLOW_HP2_tex(uv);
    return ((k % 2 == 0) ? t.xy : t.zw) * 2.0 * HOOKED_pt;
}
// full-res flow slot k -> k+1 (fwd) or k+1 -> k (bwd), in uv
vec2 flow_f(int k, bool fwd, vec2 uv) {
    vec4 t = (k == 0) ? FLOW_FP0_tex(uv) : (k == 1) ? FLOW_FP1_tex(uv) : (k == 2) ? FLOW_FP2_tex(uv) : (k == 3) ? FLOW_FP3_tex(uv) : FLOW_FP4_tex(uv);
    return (fwd ? t.xy : t.zw) * HOOKED_pt;
}
float cut(int k) {
    vec4 c = CUTS_tex(vec2((k < 4) ? 0.25 : 0.75, 0.5));
    return (k == 0) ? c.r : (k == 1) ? c.g : (k == 2) ? c.b : (k == 3) ? c.a : c.r;
}
...
vec4 hook() {
    ...
    int anchor = clamp(s <= 0.5 ? p : p + 1, 1, 4);
    if (TRI_DIAG != 0 && DIAG_HOLD_ANCHOR == 1) anchor = 3;
    ...
    // ---- the FIELD: a weighted least-squares quartic through every trusted link, up to six ----
    ...
    if (TRI_DIAG != 0) {
        ...
        if (TRI_DIAG == 6)
            return vec4(vec3(clamp(resid_px / RESID_DIAG_FS, 0.0, 1.0)), 1.0);
...
// quadratic (two) or zero as links fall away. The WARP always uses the
// cubic, so the picture equals the quad's.
int TRI_DIAG = (read_view == 1 || read_view == 4 || read_view >= 7) ? 7 : (read_view == 2 || read_view == 5) ? 2 : 5;
const float ACCEL_DIAG_FS = 2.0;
const float CORR_DIAG_FS  = 0.25;
const float JERK_DIAG_FS  = 2.0;
const float RESID_DIAG_FS = 2.0;
...
vec4 hook() {
    ...
    if (TRI_DIAG != 0) {
        ...
        if (TRI_DIAG == 8)
            return vec4(accel_c / HOOKED_pt, 0.0, 1.0);
        if (TRI_DIAG == 9)
            return vec4(jerk_c / HOOKED_pt, 0.0, 1.0);
        float m = clamp(length(accel / HOOKED_pt) / ACCEL_DIAG_FS, 0.0, 1.0);
        vec3 c = m < 0.5 ? mix(vec3(0.0, 0.0, 0.25), vec3(0.0, 0.9, 0.9), m * 2.0)
                         : mix(vec3(0.0, 0.9, 0.9), vec3(1.0, 0.1, 0.0), (m - 0.5) * 2.0);
        return vec4(c, 1.0);
...
//!HEIGHT HOOKED.h 64 /
//!WHEN read_view 0 >
//!DESC [reading] the pooled field's largest magnitude in each 8x8 block of cells: the first stage of the frame's maximum
...
//!WHEN read_view 0 >
//!DESC [reading] the frame's maximum of the pooled field with a memory: the auto scale of the painting's opacity (read_alpha 0). One texel.
