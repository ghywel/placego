...
//!HEIGHT 1
//!COMPONENTS 2
//!DESC [quad] scene-cut statistic (.r) and held-copy statistic (.g), slots 0-1
vec4 hook() {
    const int N = 24;
    float acc = 0.0;
    for (int y = 0; y < N; y++) {
    ...
    float peak = 0.0;
    ivec2 n = ivec2(1.0 / LUMA_A_S_pt + 0.5);
    for (int y = 0; y < n.y; y++) {
        for (int x = 0; x < n.x; x++) {
            vec2 uv = (vec2(float(x), float(y)) + 0.5) * LUMA_A_S_pt;
            peak = max(peak, abs(LUMA_A_S_tex(uv).r - LUMA_B_S_tex(uv).r));
        }
    }
...
//!HEIGHT 1
//!COMPONENTS 2
//!DESC [quad] scene-cut statistic (.r) and held-copy statistic (.g), slots 1-2
vec4 hook() {
    const int N = 24;
    float acc = 0.0;
    for (int y = 0; y < N; y++) {
    ...
    float peak = 0.0;
    ivec2 n = ivec2(1.0 / LUMA_B_S_pt + 0.5);
    for (int y = 0; y < n.y; y++) {
        for (int x = 0; x < n.x; x++) {
            vec2 uv = (vec2(float(x), float(y)) + 0.5) * LUMA_B_S_pt;
            peak = max(peak, abs(LUMA_B_S_tex(uv).r - LUMA_C_S_tex(uv).r));
        }
    }
    return vec4(acc / float(N * N), peak, 0.0, 0.0);
}
...
//!HEIGHT 1
//!COMPONENTS 2
//!DESC [quad] scene-cut statistic (.r) and held-copy statistic (.g), slots 2-3
vec4 hook() {
    const int N = 24;
    float acc = 0.0;
    for (int y = 0; y < N; y++) {
    ...
    float peak = 0.0;
    ivec2 n = ivec2(1.0 / LUMA_C_S_pt + 0.5);
    for (int y = 0; y < n.y; y++) {
        for (int x = 0; x < n.x; x++) {
            vec2 uv = (vec2(float(x), float(y)) + 0.5) * LUMA_C_S_pt;
            peak = max(peak, abs(LUMA_C_S_tex(uv).r - LUMA_D_S_tex(uv).r));
        }
    }
    return vec4(acc / float(N * N), peak, 0.0, 0.0);
}
...
//!HEIGHT HOOKED.h 16 /
//!COMPONENTS 4
//!DESC [quad] coarse flow search slot2->slot3 and slot3->slot2 (1/16 res) [fused: one dispatch]
...
//!HEIGHT HOOKED.h 2 /
//!COMPONENTS 2
//!DESC [quad] vector median filter on flow slot2->slot3 (pass 1) [fused with its B->A twin: one dispatch]
void hook_ba() {
    if (!pair_changed)
...
//!HEIGHT HOOKED.h 16 /
//!COMPONENTS 4
//!DESC [quad] coarse flow search slot1->slot2 and slot2->slot1 (1/16 res) [fused: one dispatch] [foresight: fourth descent from the CD/DC 1/8 flow]
...
//!HEIGHT HOOKED.h 8 /
//!COMPONENTS 2
//!DESC [quad] refine flow slot1->slot2 (1/8 res) [foresight: fifth candidate and prior from the CD 1/8 flow]
...
//!HEIGHT HOOKED.h 8 /
//!COMPONENTS 2
//!DESC [quad] refine flow slot2->slot1 (1/8 res) [foresight: fifth candidate and prior from the DC 1/8 flow]
...
//!HEIGHT HOOKED.h 2 /
//!COMPONENTS 2
//!DESC [quad] vector median filter on flow slot1->slot2 (pass 1) [fused with its B->A twin: one dispatch]
void hook_ba() {
    if (!pair_changed)
...
vec4 hook() {
    ...
    // ---- the cadence: the drawing changes less often than the frame ----
