...
vec4 hook() {
    ...
    vec2 uv_a = LUMA_A_E_pos;
    vec2 base_off = FLOW_S_CD_tex(snap_texel(uv_a, FLOW_S_CD_size)).xy * 2.0 * LUMA_A_E_pt;
...
vec4 hook() {
    ...
    vec2 uv_b = LUMA_B_E_pos;
    vec2 base_off = FLOW_S_DC_tex(snap_texel(uv_b, FLOW_S_DC_size)).xy * 2.0 * LUMA_A_E_pt;
