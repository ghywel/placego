...
//
// Produced by scripts/tests/scale_shader.py from bidirectional-interpolation-variational.glsl
// with factors 2 (S x2, E x2, Q x2, H x2), 2026-09-07. It is that shader with each
...
// with 4.0 instead of 2.0, and every constant in the frame's pixels is
// 2 times the original's -- so the field is in this frame's own pixels
// and a painted or machine-read field is comparable with the original's
// at 2 times its full scale. Storage caches were sized for 4K at the
// original divisors, so this file is safe to 2 times 4K. The comments
// below describe the pyramid in the original shader's terms ("1/16
// resolution" is 1/32 of this frame, and so on).
//
//   ./scale_shader.py bidirectional-interpolation-variational.glsl bidirectional-interpolation-variational-4k.glsl 2
// =====================================================================
...
//
//   ./gen_variational.py "16,12,8,4" 0.3 0.08 <output.glsl> 0 "2,2,2,0"
//
// Variational iterations per pyramid level: S=16 E=12 Q=8 H=4
// (S = 1/16 resolution, E = 1/8, Q = 1/4, H = 1/2). Roughly 3.5
// full-resolution pass-equivalents of added work.
//
// Vector-median passes per level: S=2 E=2 Q=2 H=0, on top of the
...
// ---------------------------------------------------------------------
// VARIATIONAL REFINEMENT at S (16x downsampled), 16 iterations per
...
//
// One texel of propagation per iteration means 256px of reach at this
...
//!HEIGHT HOOKED.h 32 /
//!COMPONENTS 2
//!DESC [high] variational S A->B (iter 1)
const float VAR_ALPHA = 0.3;
const float VAR_SIGMA_LUMA = 0.08;
vec4 hook() {
    vec2 uv = FLOW_S_AB_pos;
    vec2 f0 = FLOW_S_AB_tex(uv).xy;
    vec2 pt = LUMA_A_S_pt;

    float cl = LUMA_A_S_tex(uv).r;
    vec2 acc = vec2(0.0);
    float wsum = 0.0;
    for (int y = -1; y <= 1; y++) {
        for (int x = -1; x <= 1; x++) {
    ...
    vec2 wuv = uv + f0 * pt;
    float It = LUMA_B_S_tex(wuv).r - LUMA_A_S_tex(uv).r;
    float Ix = (LUMA_B_S_tex(wuv + vec2(pt.x, 0.0)).r - LUMA_B_S_tex(wuv - vec2(pt.x, 0.0)).r) * 0.5;
    float Iy = (LUMA_B_S_tex(wuv + vec2(0.0, pt.y)).r - LUMA_B_S_tex(wuv - vec2(0.0, pt.y)).r) * 0.5;
...
//!HEIGHT HOOKED.h 32 /
//!COMPONENTS 2
//!DESC [high] variational S A->B (iter 2)
const float VAR_ALPHA = 0.3;
const float VAR_SIGMA_LUMA = 0.08;
vec4 hook() {
    vec2 uv = FLOW_S_AB_pos;
...
//!HEIGHT HOOKED.h 32 /
//!COMPONENTS 2
//!DESC [high] variational S A->B (iter 3)
const float VAR_ALPHA = 0.3;
const float VAR_SIGMA_LUMA = 0.08;
vec4 hook() {
    vec2 uv = FLOW_S_AB_pos;
...
//!HEIGHT HOOKED.h 32 /
//!COMPONENTS 2
//!DESC [high] variational S A->B (iter 4)
const float VAR_ALPHA = 0.3;
const float VAR_SIGMA_LUMA = 0.08;
vec4 hook() {
    vec2 uv = FLOW_S_AB_pos;
...
//!HEIGHT HOOKED.h 32 /
//!COMPONENTS 2
//!DESC [high] variational S A->B (iter 5)
const float VAR_ALPHA = 0.3;
const float VAR_SIGMA_LUMA = 0.08;
vec4 hook() {
    vec2 uv = FLOW_S_AB_pos;
...
//!HEIGHT HOOKED.h 32 /
//!COMPONENTS 2
//!DESC [high] variational S A->B (iter 6)
const float VAR_ALPHA = 0.3;
const float VAR_SIGMA_LUMA = 0.08;
vec4 hook() {
    vec2 uv = FLOW_S_AB_pos;
...
//!HEIGHT HOOKED.h 32 /
//!COMPONENTS 2
//!DESC [high] variational S A->B (iter 7)
const float VAR_ALPHA = 0.3;
const float VAR_SIGMA_LUMA = 0.08;
vec4 hook() {
    vec2 uv = FLOW_S_AB_pos;
...
//!HEIGHT HOOKED.h 32 /
//!COMPONENTS 2
//!DESC [high] variational S A->B (iter 8)
const float VAR_ALPHA = 0.3;
const float VAR_SIGMA_LUMA = 0.08;
vec4 hook() {
    vec2 uv = FLOW_S_AB_pos;
...
//!HEIGHT HOOKED.h 32 /
//!COMPONENTS 2
//!DESC [high] variational S A->B (iter 9)
const float VAR_ALPHA = 0.3;
const float VAR_SIGMA_LUMA = 0.08;
vec4 hook() {
    vec2 uv = FLOW_S_AB_pos;
...
//!HEIGHT HOOKED.h 32 /
//!COMPONENTS 2
//!DESC [high] variational S A->B (iter 10)
const float VAR_ALPHA = 0.3;
const float VAR_SIGMA_LUMA = 0.08;
vec4 hook() {
    vec2 uv = FLOW_S_AB_pos;
...
//!HEIGHT HOOKED.h 32 /
//!COMPONENTS 2
//!DESC [high] variational S A->B (iter 11)
const float VAR_ALPHA = 0.3;
const float VAR_SIGMA_LUMA = 0.08;
vec4 hook() {
    vec2 uv = FLOW_S_AB_pos;
...
//!HEIGHT HOOKED.h 32 /
//!COMPONENTS 2
//!DESC [high] variational S A->B (iter 12)
const float VAR_ALPHA = 0.3;
const float VAR_SIGMA_LUMA = 0.08;
vec4 hook() {
    vec2 uv = FLOW_S_AB_pos;
...
//!HEIGHT HOOKED.h 32 /
//!COMPONENTS 2
//!DESC [high] variational S A->B (iter 13)
const float VAR_ALPHA = 0.3;
const float VAR_SIGMA_LUMA = 0.08;
vec4 hook() {
    vec2 uv = FLOW_S_AB_pos;
...
//!HEIGHT HOOKED.h 32 /
//!COMPONENTS 2
//!DESC [high] variational S A->B (iter 14)
const float VAR_ALPHA = 0.3;
const float VAR_SIGMA_LUMA = 0.08;
vec4 hook() {
    vec2 uv = FLOW_S_AB_pos;
...
//!HEIGHT HOOKED.h 32 /
//!COMPONENTS 2
//!DESC [high] variational S A->B (iter 15)
const float VAR_ALPHA = 0.3;
const float VAR_SIGMA_LUMA = 0.08;
vec4 hook() {
    vec2 uv = FLOW_S_AB_pos;
...
//!HEIGHT HOOKED.h 32 /
//!COMPONENTS 2
//!DESC [high] variational S A->B (iter 16)
const float VAR_ALPHA = 0.3;
const float VAR_SIGMA_LUMA = 0.08;
vec4 hook() {
    vec2 uv = FLOW_S_AB_pos;
...
//!HEIGHT HOOKED.h 32 /
//!COMPONENTS 2
//!DESC [high] variational S B->A (iter 1)
const float VAR_ALPHA = 0.3;
const float VAR_SIGMA_LUMA = 0.08;
vec4 hook() {
    vec2 uv = FLOW_S_BA_pos;
    vec2 f0 = FLOW_S_BA_tex(uv).xy;
    vec2 pt = LUMA_B_S_pt;

    float cl = LUMA_B_S_tex(uv).r;
    vec2 acc = vec2(0.0);
    float wsum = 0.0;
    for (int y = -1; y <= 1; y++) {
        for (int x = -1; x <= 1; x++) {
            if (x == 0 && y == 0) continue;
            vec2 o = vec2(float(x), float(y)) * pt;
            float dl = LUMA_B_S_tex(uv + o).r - cl;
            float ws = (x == 0 || y == 0) ? 1.0 : 0.70710678;
            float w = ws * exp(-(dl * dl) / (2.0 * VAR_SIGMA_LUMA * VAR_SIGMA_LUMA));
            vec2 nf = FLOW_S_BA_tex(uv + o).xy;
            acc += nf * w;
            wsum += w;
        }
    }
    ...
    vec2 wuv = uv + f0 * pt;
    float It = LUMA_A_S_tex(wuv).r - LUMA_B_S_tex(uv).r;
    float Ix = (LUMA_A_S_tex(wuv + vec2(pt.x, 0.0)).r - LUMA_A_S_tex(wuv - vec2(pt.x, 0.0)).r) * 0.5;
    float Iy = (LUMA_A_S_tex(wuv + vec2(0.0, pt.y)).r - LUMA_A_S_tex(wuv - vec2(0.0, pt.y)).r) * 0.5;
...
//!HEIGHT HOOKED.h 32 /
//!COMPONENTS 2
//!DESC [high] variational S B->A (iter 2)
const float VAR_ALPHA = 0.3;
const float VAR_SIGMA_LUMA = 0.08;
vec4 hook() {
    vec2 uv = FLOW_S_BA_pos;
...
//!HEIGHT HOOKED.h 32 /
//!COMPONENTS 2
//!DESC [high] variational S B->A (iter 3)
const float VAR_ALPHA = 0.3;
const float VAR_SIGMA_LUMA = 0.08;
vec4 hook() {
    vec2 uv = FLOW_S_BA_pos;
...
//!HEIGHT HOOKED.h 32 /
//!COMPONENTS 2
//!DESC [high] variational S B->A (iter 4)
const float VAR_ALPHA = 0.3;
const float VAR_SIGMA_LUMA = 0.08;
vec4 hook() {
    vec2 uv = FLOW_S_BA_pos;
...
//!HEIGHT HOOKED.h 32 /
//!COMPONENTS 2
//!DESC [high] variational S B->A (iter 5)
const float VAR_ALPHA = 0.3;
const float VAR_SIGMA_LUMA = 0.08;
vec4 hook() {
    vec2 uv = FLOW_S_BA_pos;
...
//!HEIGHT HOOKED.h 32 /
//!COMPONENTS 2
//!DESC [high] variational S B->A (iter 6)
const float VAR_ALPHA = 0.3;
const float VAR_SIGMA_LUMA = 0.08;
vec4 hook() {
    vec2 uv = FLOW_S_BA_pos;
...
//!HEIGHT HOOKED.h 32 /
//!COMPONENTS 2
//!DESC [high] variational S B->A (iter 7)
const float VAR_ALPHA = 0.3;
const float VAR_SIGMA_LUMA = 0.08;
vec4 hook() {
    vec2 uv = FLOW_S_BA_pos;
...
//!HEIGHT HOOKED.h 32 /
//!COMPONENTS 2
//!DESC [high] variational S B->A (iter 8)
const float VAR_ALPHA = 0.3;
const float VAR_SIGMA_LUMA = 0.08;
vec4 hook() {
    vec2 uv = FLOW_S_BA_pos;
...
//!HEIGHT HOOKED.h 32 /
//!COMPONENTS 2
//!DESC [high] variational S B->A (iter 9)
const float VAR_ALPHA = 0.3;
const float VAR_SIGMA_LUMA = 0.08;
vec4 hook() {
    vec2 uv = FLOW_S_BA_pos;
...
//!HEIGHT HOOKED.h 32 /
//!COMPONENTS 2
//!DESC [high] variational S B->A (iter 10)
const float VAR_ALPHA = 0.3;
const float VAR_SIGMA_LUMA = 0.08;
vec4 hook() {
    vec2 uv = FLOW_S_BA_pos;
...
//!HEIGHT HOOKED.h 32 /
//!COMPONENTS 2
//!DESC [high] variational S B->A (iter 11)
const float VAR_ALPHA = 0.3;
const float VAR_SIGMA_LUMA = 0.08;
vec4 hook() {
    vec2 uv = FLOW_S_BA_pos;
...
//!HEIGHT HOOKED.h 32 /
//!COMPONENTS 2
//!DESC [high] variational S B->A (iter 12)
const float VAR_ALPHA = 0.3;
const float VAR_SIGMA_LUMA = 0.08;
vec4 hook() {
    vec2 uv = FLOW_S_BA_pos;
...
//!HEIGHT HOOKED.h 32 /
//!COMPONENTS 2
//!DESC [high] variational S B->A (iter 13)
const float VAR_ALPHA = 0.3;
const float VAR_SIGMA_LUMA = 0.08;
vec4 hook() {
    vec2 uv = FLOW_S_BA_pos;
...
//!HEIGHT HOOKED.h 32 /
//!COMPONENTS 2
//!DESC [high] variational S B->A (iter 14)
const float VAR_ALPHA = 0.3;
const float VAR_SIGMA_LUMA = 0.08;
vec4 hook() {
    vec2 uv = FLOW_S_BA_pos;
...
//!HEIGHT HOOKED.h 32 /
//!COMPONENTS 2
//!DESC [high] variational S B->A (iter 15)
const float VAR_ALPHA = 0.3;
const float VAR_SIGMA_LUMA = 0.08;
vec4 hook() {
    vec2 uv = FLOW_S_BA_pos;
...
//!HEIGHT HOOKED.h 32 /
//!COMPONENTS 2
//!DESC [high] variational S B->A (iter 16)
const float VAR_ALPHA = 0.3;
const float VAR_SIGMA_LUMA = 0.08;
vec4 hook() {
    vec2 uv = FLOW_S_BA_pos;
...
// ---------------------------------------------------------------------
// VECTOR MEDIAN at S (16x downsampled), 2 pass(es) per direction.
//
...
//!HEIGHT HOOKED.h 32 /
//!COMPONENTS 2
//!DESC [high] vector median S A->B (pass 1)
vec4 hook() {
    vec2 v[9];
    int n = 0;
    for (int y = -1; y <= 1; y++) {
        for (int x = -1; x <= 1; x++) {
            vec2 o = vec2(float(x), float(y)) * FLOW_S_AB_pt;
            v[n++] = FLOW_S_AB_tex(FLOW_S_AB_pos + o).xy;
        }
    }
...
//!HEIGHT HOOKED.h 32 /
//!COMPONENTS 2
//!DESC [high] vector median S A->B (pass 2)
vec4 hook() {
    vec2 v[9];
    int n = 0;
    for (int y = -1; y <= 1; y++) {
...
//!HEIGHT HOOKED.h 32 /
//!COMPONENTS 2
//!DESC [high] vector median S B->A (pass 1)
vec4 hook() {
    vec2 v[9];
    int n = 0;
    for (int y = -1; y <= 1; y++) {
        for (int x = -1; x <= 1; x++) {
            vec2 o = vec2(float(x), float(y)) * FLOW_S_BA_pt;
            v[n++] = FLOW_S_BA_tex(FLOW_S_BA_pos + o).xy;
        }
    }
...
//!HEIGHT HOOKED.h 32 /
//!COMPONENTS 2
//!DESC [high] vector median S B->A (pass 2)
vec4 hook() {
    vec2 v[9];
    int n = 0;
    for (int y = -1; y <= 1; y++) {
...
// ---------------------------------------------------------------------
// VARIATIONAL REFINEMENT at E (8x downsampled), 12 iterations per
// direction. Warped Horn-Schunck with edge-aware smoothness.
//
// Coherence enters the OBJECTIVE here rather than being imposed afterwards:
// each iteration jointly minimises brightness-constancy residual and
...
//   f_new = favg - (Ix,Iy) * rho / (alpha^2 + Ix^2 + Iy^2)
//
// One texel of propagation per iteration means 96px of reach at this
// level, which is the whole reason the iterations are spread across the
// pyramid rather than concentrated at the finest level.
// ---------------------------------------------------------------------
//!HOOK FRAME_MIX
...
//!HEIGHT HOOKED.h 16 /
//!COMPONENTS 2
//!DESC [high] variational E A->B (iter 1)
const float VAR_ALPHA = 0.3;
const float VAR_SIGMA_LUMA = 0.08;
vec4 hook() {
    vec2 uv = FLOW_E_AB_pos;
    vec2 f0 = FLOW_E_AB_tex(uv).xy;
    vec2 pt = LUMA_A_E_pt;

    float cl = LUMA_A_E_tex(uv).r;
    vec2 acc = vec2(0.0);
    float wsum = 0.0;
    for (int y = -1; y <= 1; y++) {
        for (int x = -1; x <= 1; x++) {
            if (x == 0 && y == 0) continue;
            vec2 o = vec2(float(x), float(y)) * pt;
            float dl = LUMA_A_E_tex(uv + o).r - cl;
            float ws = (x == 0 || y == 0) ? 1.0 : 0.70710678;
            float w = ws * exp(-(dl * dl) / (2.0 * VAR_SIGMA_LUMA * VAR_SIGMA_LUMA));
            vec2 nf = FLOW_E_AB_tex(uv + o).xy;
            acc += nf * w;
            wsum += w;
        }
    }
    ...
    vec2 wuv = uv + f0 * pt;
    float It = LUMA_B_E_tex(wuv).r - LUMA_A_E_tex(uv).r;
    float Ix = (LUMA_B_E_tex(wuv + vec2(pt.x, 0.0)).r - LUMA_B_E_tex(wuv - vec2(pt.x, 0.0)).r) * 0.5;
    float Iy = (LUMA_B_E_tex(wuv + vec2(0.0, pt.y)).r - LUMA_B_E_tex(wuv - vec2(0.0, pt.y)).r) * 0.5;
...
//!HEIGHT HOOKED.h 16 /
//!COMPONENTS 2
//!DESC [high] variational E A->B (iter 2)
const float VAR_ALPHA = 0.3;
const float VAR_SIGMA_LUMA = 0.08;
vec4 hook() {
    vec2 uv = FLOW_E_AB_pos;
...
//!HEIGHT HOOKED.h 16 /
//!COMPONENTS 2
//!DESC [high] variational E A->B (iter 3)
const float VAR_ALPHA = 0.3;
const float VAR_SIGMA_LUMA = 0.08;
vec4 hook() {
    vec2 uv = FLOW_E_AB_pos;
...
//!HEIGHT HOOKED.h 16 /
//!COMPONENTS 2
//!DESC [high] variational E A->B (iter 4)
const float VAR_ALPHA = 0.3;
const float VAR_SIGMA_LUMA = 0.08;
vec4 hook() {
    vec2 uv = FLOW_E_AB_pos;
...
//!HEIGHT HOOKED.h 16 /
//!COMPONENTS 2
//!DESC [high] variational E A->B (iter 5)
const float VAR_ALPHA = 0.3;
const float VAR_SIGMA_LUMA = 0.08;
vec4 hook() {
    vec2 uv = FLOW_E_AB_pos;
...
//!HEIGHT HOOKED.h 16 /
//!COMPONENTS 2
//!DESC [high] variational E A->B (iter 6)
const float VAR_ALPHA = 0.3;
const float VAR_SIGMA_LUMA = 0.08;
vec4 hook() {
    vec2 uv = FLOW_E_AB_pos;
...
//!HEIGHT HOOKED.h 16 /
//!COMPONENTS 2
//!DESC [high] variational E A->B (iter 7)
const float VAR_ALPHA = 0.3;
const float VAR_SIGMA_LUMA = 0.08;
vec4 hook() {
    vec2 uv = FLOW_E_AB_pos;
...
//!HEIGHT HOOKED.h 16 /
//!COMPONENTS 2
//!DESC [high] variational E A->B (iter 8)
const float VAR_ALPHA = 0.3;
const float VAR_SIGMA_LUMA = 0.08;
vec4 hook() {
    vec2 uv = FLOW_E_AB_pos;
...
//!HEIGHT HOOKED.h 16 /
//!COMPONENTS 2
//!DESC [high] variational E A->B (iter 9)
const float VAR_ALPHA = 0.3;
const float VAR_SIGMA_LUMA = 0.08;
vec4 hook() {
    vec2 uv = FLOW_E_AB_pos;
...
//!HEIGHT HOOKED.h 16 /
//!COMPONENTS 2
//!DESC [high] variational E A->B (iter 10)
const float VAR_ALPHA = 0.3;
const float VAR_SIGMA_LUMA = 0.08;
vec4 hook() {
    vec2 uv = FLOW_E_AB_pos;
...
//!HEIGHT HOOKED.h 16 /
//!COMPONENTS 2
//!DESC [high] variational E A->B (iter 11)
const float VAR_ALPHA = 0.3;
const float VAR_SIGMA_LUMA = 0.08;
vec4 hook() {
    vec2 uv = FLOW_E_AB_pos;
...
//!HEIGHT HOOKED.h 16 /
//!COMPONENTS 2
//!DESC [high] variational E A->B (iter 12)
const float VAR_ALPHA = 0.3;
const float VAR_SIGMA_LUMA = 0.08;
vec4 hook() {
    vec2 uv = FLOW_E_AB_pos;
...
//!HEIGHT HOOKED.h 16 /
//!COMPONENTS 2
//!DESC [high] variational E B->A (iter 1)
const float VAR_ALPHA = 0.3;
const float VAR_SIGMA_LUMA = 0.08;
vec4 hook() {
    vec2 uv = FLOW_E_BA_pos;
    vec2 f0 = FLOW_E_BA_tex(uv).xy;
    vec2 pt = LUMA_B_E_pt;

    float cl = LUMA_B_E_tex(uv).r;
    vec2 acc = vec2(0.0);
    float wsum = 0.0;
    for (int y = -1; y <= 1; y++) {
        for (int x = -1; x <= 1; x++) {
            if (x == 0 && y == 0) continue;
            vec2 o = vec2(float(x), float(y)) * pt;
            float dl = LUMA_B_E_tex(uv + o).r - cl;
            float ws = (x == 0 || y == 0) ? 1.0 : 0.70710678;
            float w = ws * exp(-(dl * dl) / (2.0 * VAR_SIGMA_LUMA * VAR_SIGMA_LUMA));
            vec2 nf = FLOW_E_BA_tex(uv + o).xy;
            acc += nf * w;
            wsum += w;
        }
    }
    ...
    vec2 wuv = uv + f0 * pt;
    float It = LUMA_A_E_tex(wuv).r - LUMA_B_E_tex(uv).r;
    float Ix = (LUMA_A_E_tex(wuv + vec2(pt.x, 0.0)).r - LUMA_A_E_tex(wuv - vec2(pt.x, 0.0)).r) * 0.5;
    float Iy = (LUMA_A_E_tex(wuv + vec2(0.0, pt.y)).r - LUMA_A_E_tex(wuv - vec2(0.0, pt.y)).r) * 0.5;
...
//!HEIGHT HOOKED.h 16 /
//!COMPONENTS 2
//!DESC [high] variational E B->A (iter 2)
const float VAR_ALPHA = 0.3;
const float VAR_SIGMA_LUMA = 0.08;
vec4 hook() {
    vec2 uv = FLOW_E_BA_pos;
...
//!HEIGHT HOOKED.h 16 /
//!COMPONENTS 2
//!DESC [high] variational E B->A (iter 3)
const float VAR_ALPHA = 0.3;
const float VAR_SIGMA_LUMA = 0.08;
vec4 hook() {
    vec2 uv = FLOW_E_BA_pos;
...
//!HEIGHT HOOKED.h 16 /
//!COMPONENTS 2
//!DESC [high] variational E B->A (iter 4)
const float VAR_ALPHA = 0.3;
const float VAR_SIGMA_LUMA = 0.08;
vec4 hook() {
    vec2 uv = FLOW_E_BA_pos;
...
//!HEIGHT HOOKED.h 16 /
//!COMPONENTS 2
//!DESC [high] variational E B->A (iter 5)
const float VAR_ALPHA = 0.3;
const float VAR_SIGMA_LUMA = 0.08;
vec4 hook() {
    vec2 uv = FLOW_E_BA_pos;
...
//!HEIGHT HOOKED.h 16 /
//!COMPONENTS 2
//!DESC [high] variational E B->A (iter 6)
const float VAR_ALPHA = 0.3;
const float VAR_SIGMA_LUMA = 0.08;
vec4 hook() {
    vec2 uv = FLOW_E_BA_pos;
...
//!HEIGHT HOOKED.h 16 /
//!COMPONENTS 2
//!DESC [high] variational E B->A (iter 7)
const float VAR_ALPHA = 0.3;
const float VAR_SIGMA_LUMA = 0.08;
vec4 hook() {
    vec2 uv = FLOW_E_BA_pos;
...
//!HEIGHT HOOKED.h 16 /
//!COMPONENTS 2
//!DESC [high] variational E B->A (iter 8)
const float VAR_ALPHA = 0.3;
const float VAR_SIGMA_LUMA = 0.08;
vec4 hook() {
    vec2 uv = FLOW_E_BA_pos;
...
//!HEIGHT HOOKED.h 16 /
//!COMPONENTS 2
//!DESC [high] variational E B->A (iter 9)
const float VAR_ALPHA = 0.3;
const float VAR_SIGMA_LUMA = 0.08;
vec4 hook() {
    vec2 uv = FLOW_E_BA_pos;
...
//!HEIGHT HOOKED.h 16 /
//!COMPONENTS 2
//!DESC [high] variational E B->A (iter 10)
const float VAR_ALPHA = 0.3;
const float VAR_SIGMA_LUMA = 0.08;
vec4 hook() {
    vec2 uv = FLOW_E_BA_pos;
...
//!HEIGHT HOOKED.h 16 /
//!COMPONENTS 2
//!DESC [high] variational E B->A (iter 11)
const float VAR_ALPHA = 0.3;
const float VAR_SIGMA_LUMA = 0.08;
vec4 hook() {
    vec2 uv = FLOW_E_BA_pos;
...
//!HEIGHT HOOKED.h 16 /
//!COMPONENTS 2
//!DESC [high] variational E B->A (iter 12)
const float VAR_ALPHA = 0.3;
const float VAR_SIGMA_LUMA = 0.08;
vec4 hook() {
    vec2 uv = FLOW_E_BA_pos;
...
// ---------------------------------------------------------------------
// VECTOR MEDIAN at E (8x downsampled), 2 pass(es) per direction.
//
// The base shader already medians the flow, but only at H and only 3x3 (twice,
// so about 5x5 of reach). That is enough for a stray texel and useless against
// what actually goes wrong on flat-shaded animation: small high-contrast
...
//!HEIGHT HOOKED.h 16 /
//!COMPONENTS 2
//!DESC [high] vector median E A->B (pass 1)
vec4 hook() {
    vec2 v[9];
    int n = 0;
    for (int y = -1; y <= 1; y++) {
        for (int x = -1; x <= 1; x++) {
            vec2 o = vec2(float(x), float(y)) * FLOW_E_AB_pt;
            v[n++] = FLOW_E_AB_tex(FLOW_E_AB_pos + o).xy;
        }
    }
...
//!HEIGHT HOOKED.h 16 /
//!COMPONENTS 2
//!DESC [high] vector median E A->B (pass 2)
vec4 hook() {
    vec2 v[9];
    int n = 0;
    for (int y = -1; y <= 1; y++) {
...
//!HEIGHT HOOKED.h 16 /
//!COMPONENTS 2
//!DESC [high] vector median E B->A (pass 1)
vec4 hook() {
    vec2 v[9];
    int n = 0;
    for (int y = -1; y <= 1; y++) {
        for (int x = -1; x <= 1; x++) {
            vec2 o = vec2(float(x), float(y)) * FLOW_E_BA_pt;
            v[n++] = FLOW_E_BA_tex(FLOW_E_BA_pos + o).xy;
        }
    }
...
//!HEIGHT HOOKED.h 16 /
//!COMPONENTS 2
//!DESC [high] vector median E B->A (pass 2)
vec4 hook() {
    vec2 v[9];
    int n = 0;
    for (int y = -1; y <= 1; y++) {
...
vec4 hook() {
    ...
    vec2 flow_ab = FLOW_H_AB_tex(HOOKED_pos).xy * 4.0 * HOOKED_pt;
...
const float READ_MACHINE_FS_VEL = 64.0;
const float READ_MACHINE_FS_ACC = 4.0;
const float READ_MACHINE_FS_JERK = 4.0;
const float READ_MACHINE_FS_DERIV = 1.0;    // 1/frame: divergence, curl, shear
