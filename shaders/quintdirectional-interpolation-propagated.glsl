...
//!HEIGHT HOOKED.h 8 /
//!COMPONENTS 2
//!DESC [prop12] contrast-weighted flow propagation AB (pass 1 of 3)
...
//!HEIGHT HOOKED.h 8 /
//!COMPONENTS 2
//!DESC [prop12] contrast-weighted flow propagation AB (pass 2 of 3)
...
//!HEIGHT HOOKED.h 8 /
//!COMPONENTS 2
//!DESC [prop12] contrast-weighted flow propagation AB (pass 3 of 3)
...
//!HEIGHT HOOKED.h 8 /
//!COMPONENTS 2
//!DESC [prop12] three-way data check AB: propagated vs raw vs zero, ties to consensus on texture only
...
//!HEIGHT HOOKED.h 8 /
//!COMPONENTS 2
//!DESC [prop12] contrast-weighted flow propagation BA (pass 1 of 3)
...
//!HEIGHT HOOKED.h 8 /
//!COMPONENTS 2
//!DESC [prop12] contrast-weighted flow propagation BA (pass 2 of 3)
...
//!HEIGHT HOOKED.h 8 /
//!COMPONENTS 2
//!DESC [prop12] contrast-weighted flow propagation BA (pass 3 of 3)
...
//!HEIGHT HOOKED.h 8 /
//!COMPONENTS 2
//!DESC [prop12] three-way data check BA: propagated vs raw vs zero, ties to consensus on texture only
...
//!HEIGHT HOOKED.h 16 /
//!COMPONENTS 4
//!DESC [quint] coarse flow search slot1->slot2 (1/16 res)
...
//!HEIGHT HOOKED.h 2 /
//!COMPONENTS 2
//!DESC [quint] vector median filter on flow slot1->slot2 (pass 1)
vec4 hook() {
    if (!pair_changed)
        return vec4(0.0);
...
//!HEIGHT HOOKED.h 16 /
//!COMPONENTS 4
//!DESC [quint] coarse flow search slot2->slot1 (1/16 res)
...
//!HEIGHT HOOKED.h 2 /
//!COMPONENTS 2
//!DESC [quint] vector median filter on flow slot2->slot1 (pass 1)
vec4 hook() {
    if (!pair_changed)
        return vec4(0.0);
...
//!HEIGHT HOOKED.h 16 /
//!COMPONENTS 4
//!DESC [quint] coarse flow search slot2->slot3 (1/16 res)
...
//!HEIGHT HOOKED.h 2 /
//!COMPONENTS 2
//!DESC [quint] vector median filter on flow slot2->slot3 (pass 1)
vec4 hook() {
    if (!pair_changed)
        return vec4(0.0);
...
//!HEIGHT HOOKED.h 16 /
//!COMPONENTS 4
//!DESC [quint] coarse flow search slot3->slot2 (1/16 res)
...
//!HEIGHT HOOKED.h 2 /
//!COMPONENTS 2
//!DESC [quint] vector median filter on flow slot3->slot2 (pass 1)
vec4 hook() {
    if (!pair_changed)
        return vec4(0.0);
...
//!HEIGHT HOOKED.h 16 /
//!COMPONENTS 4
//!DESC [quint] coarse flow search slot3->slot4 (1/16 res)
...
//!HEIGHT HOOKED.h 2 /
//!COMPONENTS 2
//!DESC [quint] vector median filter on flow slot3->slot4 (pass 1)
vec4 hook() {
    if (!pair_changed)
        return vec4(0.0);
...
//!HEIGHT HOOKED.h 16 /
//!COMPONENTS 4
//!DESC [quint] coarse flow search slot4->slot3 (1/16 res)
...
vec4 hook() {
    ivec2 coord = ivec2(LUMA_B_S_pos * LUMA_B_S_size);
    if (!pair_changed)
        return imageLoad(FLOW_S_ED_CACHE, coord);
...
vec4 hook() {
    ...
    vec2 uv_b = LUMA_B_E_pos;
    vec4 seeds = FLOW_S_ED_tex(snap_texel(uv_b, FLOW_S_ED_size));
    vec2 base_off = seeds.xy * 2.0 * LUMA_A_E_pt;
    vec2 base_off2 = seeds.zw * 2.0 * LUMA_A_E_pt;
    ...
    bool trusted = rt < SEED_RT_MAX && length(prev_e) > 0.0;
    float tl = trusted ? SEED_TEMP_LAMBDA : 0.0;
    ivec2 scoord = ivec2(snap_texel(uv_b, FLOW_S_ED_size) * FLOW_S_ED_size);
...
vec4 hook() {
    vec2 uv = FLOW_E_ED_PROP_pos;
    vec2 own = FLOW_E_ED_PROP_tex(uv).xy;
    float c_own = prop_conf(uv);
    vec2 acc = vec2(0.0);
    float wsum = 0.0;
    for (int y = -2; y <= 2; y++)
        for (int x = -2; x <= 2; x++) {
            if (x == 0 && y == 0) continue;
            vec2 o = vec2(float(x), float(y)) * FLOW_E_ED_PROP_pt;
            float w = prop_conf(uv + o) / (1.0 + 0.5 * float(abs(x) + abs(y)));
            acc += w * FLOW_E_ED_PROP_tex(uv + o).xy;
            wsum += w;
        }
    float w_own = PROP_SELF_WEIGHT * c_own * c_own;
    if (wsum + w_own <= 0.0)
...
vec4 hook() {
    ...
    vec2 prop = FLOW_E_ED_PROP_tex(uv).xy;
    if (prop == raw)
        return vec4(raw, 0.0, 0.0);
    float s_prop = sad5(uv, prop * LUMA_B_E_pt);
    float s_raw  = sad5(uv, raw * LUMA_B_E_pt);
...
vec4 hook() {
    ...
    vec2 uv_b = LUMA_B_Q_pos;
    vec2 base_off = FLOW_E_ED_tex(snap_texel(uv_b, FLOW_E_ED_size)).xy * 2.0 * LUMA_A_Q_pt;
...
//!HEIGHT HOOKED.h 2 /
//!COMPONENTS 2
//!DESC [quint] vector median filter on flow slot4->slot3 (pass 1)
vec4 hook() {
    if (!pair_changed)
        return vec4(0.0);
...
vec4 hook() {
    ivec2 coord = ivec2(FLOW_H_ED_pos * FLOW_H_ED_size);
    if (!pair_changed)
