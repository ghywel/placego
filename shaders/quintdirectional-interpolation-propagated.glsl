...
//
//   ./tests/gen_quintdirectional.py quintdirectional-interpolation-propagated.glsl bidirectional-interpolation-propagated.glsl
//
...
//!HEIGHT HOOKED.h 16 /
//!COMPONENTS 1
//!DESC [quint] downsample slot 0 to 1/16 res (luma)
vec4 hook() {
...
//!HEIGHT HOOKED.h 16 /
//!COMPONENTS 1
//!DESC [quint] downsample slot 1 to 1/16 res (luma)
vec4 hook() {
...
//!HEIGHT HOOKED.h 16 /
//!COMPONENTS 1
//!DESC [quint] downsample slot 2 to 1/16 res (luma)
vec4 hook() {
...
//!HEIGHT HOOKED.h 16 /
//!COMPONENTS 1
//!DESC [quint] downsample slot 3 to 1/16 res (luma)
vec4 hook() {
...
//!HEIGHT HOOKED.h 16 /
//!COMPONENTS 1
//!DESC [quint] downsample slot 4 to 1/16 res (luma)
vec4 hook() {
...
//!HEIGHT 1
//!COMPONENTS 1
//!DESC [quint] scene-cut statistic, slots 0-1
vec4 hook() {
    const int N = 24;
    float acc = 0.0;
    for (int y = 0; y < N; y++) {
...
//!HEIGHT 1
//!COMPONENTS 1
//!DESC [quint] scene-cut statistic, slots 1-2
vec4 hook() {
    const int N = 24;
    float acc = 0.0;
    for (int y = 0; y < N; y++) {
...
//!HEIGHT 1
//!COMPONENTS 1
//!DESC [quint] scene-cut statistic, slots 2-3
vec4 hook() {
    const int N = 24;
    float acc = 0.0;
    for (int y = 0; y < N; y++) {
...
//!HEIGHT 1
//!COMPONENTS 1
//!DESC [quint] scene-cut statistic, slots 3-4
vec4 hook() {
    const int N = 24;
    float acc = 0.0;
    for (int y = 0; y < N; y++) {
...
//!HEIGHT HOOKED.h 8 /
//!COMPONENTS 1
//!DESC [quint] downsample slot 0 to 1/8 res (luma)
vec4 hook() {
    return vec4(dot(HOOKED_tex(HOOKED_pos).rgb, vec3(0.299, 0.587, 0.114)), 0, 0, 0);
}
...
//!HEIGHT HOOKED.h 8 /
//!COMPONENTS 1
//!DESC [quint] downsample slot 1 to 1/8 res (luma)
vec4 hook() {
    return vec4(dot(FRAME1_tex(HOOKED_pos).rgb, vec3(0.299, 0.587, 0.114)), 0, 0, 0);
}
//!HOOK FRAME_MIX
...
//!HEIGHT HOOKED.h 8 /
//!COMPONENTS 1
//!DESC [quint] downsample slot 2 to 1/8 res (luma)
vec4 hook() {
    return vec4(dot(FRAME2_tex(HOOKED_pos).rgb, vec3(0.299, 0.587, 0.114)), 0, 0, 0);
}
//!HOOK FRAME_MIX
...
//!HEIGHT HOOKED.h 8 /
//!COMPONENTS 1
//!DESC [quint] downsample slot 3 to 1/8 res (luma)
vec4 hook() {
    return vec4(dot(FRAME3_tex(HOOKED_pos).rgb, vec3(0.299, 0.587, 0.114)), 0, 0, 0);
}
//!HOOK FRAME_MIX
...
//!HEIGHT HOOKED.h 8 /
//!COMPONENTS 1
//!DESC [quint] downsample slot 4 to 1/8 res (luma)
vec4 hook() {
    return vec4(dot(FRAME4_tex(HOOKED_pos).rgb, vec3(0.299, 0.587, 0.114)), 0, 0, 0);
}
...
//!HEIGHT HOOKED.h 4 /
//!COMPONENTS 1
//!DESC [quint] downsample slot 0 to 1/4 res (luma)
vec4 hook() {
    return vec4(dot(HOOKED_tex(HOOKED_pos).rgb, vec3(0.299, 0.587, 0.114)), 0, 0, 0);
}
...
//!HEIGHT HOOKED.h 4 /
//!COMPONENTS 1
//!DESC [quint] downsample slot 1 to 1/4 res (luma)
vec4 hook() {
    return vec4(dot(FRAME1_tex(HOOKED_pos).rgb, vec3(0.299, 0.587, 0.114)), 0, 0, 0);
}
//!HOOK FRAME_MIX
...
//!HEIGHT HOOKED.h 4 /
//!COMPONENTS 1
//!DESC [quint] downsample slot 2 to 1/4 res (luma)
vec4 hook() {
    return vec4(dot(FRAME2_tex(HOOKED_pos).rgb, vec3(0.299, 0.587, 0.114)), 0, 0, 0);
}
//!HOOK FRAME_MIX
...
//!HEIGHT HOOKED.h 4 /
//!COMPONENTS 1
//!DESC [quint] downsample slot 3 to 1/4 res (luma)
vec4 hook() {
    return vec4(dot(FRAME3_tex(HOOKED_pos).rgb, vec3(0.299, 0.587, 0.114)), 0, 0, 0);
}
//!HOOK FRAME_MIX
...
//!HEIGHT HOOKED.h 4 /
//!COMPONENTS 1
//!DESC [quint] downsample slot 4 to 1/4 res (luma)
vec4 hook() {
    return vec4(dot(FRAME4_tex(HOOKED_pos).rgb, vec3(0.299, 0.587, 0.114)), 0, 0, 0);
}
...
//!HEIGHT HOOKED.h 2 /
//!COMPONENTS 1
//!DESC [quint] downsample slot 0 to half res (luma)
vec4 hook() {
    return vec4(dot(HOOKED_tex(HOOKED_pos).rgb, vec3(0.299, 0.587, 0.114)), 0, 0, 0);
}
...
//!HEIGHT HOOKED.h 2 /
//!COMPONENTS 1
//!DESC [quint] downsample slot 1 to half res (luma)
vec4 hook() {
    return vec4(dot(FRAME1_tex(HOOKED_pos).rgb, vec3(0.299, 0.587, 0.114)), 0, 0, 0);
}
//!HOOK FRAME_MIX
...
//!HEIGHT HOOKED.h 2 /
//!COMPONENTS 1
//!DESC [quint] downsample slot 2 to 1/2 res (luma)
vec4 hook() {
    return vec4(dot(FRAME2_tex(HOOKED_pos).rgb, vec3(0.299, 0.587, 0.114)), 0, 0, 0);
}
//!HOOK FRAME_MIX
...
//!HEIGHT HOOKED.h 2 /
//!COMPONENTS 1
//!DESC [quint] downsample slot 3 to 1/2 res (luma)
vec4 hook() {
    return vec4(dot(FRAME3_tex(HOOKED_pos).rgb, vec3(0.299, 0.587, 0.114)), 0, 0, 0);
}
//!HOOK FRAME_MIX
...
//!HEIGHT HOOKED.h 2 /
//!COMPONENTS 1
//!DESC [quint] downsample slot 4 to 1/2 res (luma)
vec4 hook() {
    return vec4(dot(FRAME4_tex(HOOKED_pos).rgb, vec3(0.299, 0.587, 0.114)), 0, 0, 0);
}
...
//!HEIGHT HOOKED.h 16 /
//!COMPONENTS 4
//!DESC [quint] coarse flow search slot3->slot4 and slot4->slot3 (1/16 res) [fused: one dispatch]
...
//!HEIGHT HOOKED.h 8 /
//!COMPONENTS 2
//!DESC [quint] refine flow slot3->slot4 (1/8 res)
...
//!HEIGHT HOOKED.h 8 /
//!COMPONENTS 2
//!DESC [quint] refine flow slot4->slot3 (1/8 res)
...
//!HEIGHT HOOKED.h 4 /
//!COMPONENTS 2
//!DESC [quint] refine flow slot3->slot4 (1/4 res)
...
//!HEIGHT HOOKED.h 4 /
//!COMPONENTS 2
//!DESC [quint] refine flow slot4->slot3 (1/4 res)
...
//!HEIGHT HOOKED.h 2 /
//!COMPONENTS 2
//!DESC [quint] refine flow slot3->slot4 (half res)
...
//!HEIGHT HOOKED.h 2 /
//!COMPONENTS 2
//!DESC [quint] refine flow slot4->slot3 (half res)
...
//!HEIGHT HOOKED.h 2 /
//!COMPONENTS 2
//!DESC [quint] vector median filter on flow slot3->slot4 (pass 1) [fused with its B->A twin: one dispatch]
void hook_ba() {
    if (!pair_changed)
...
//!HEIGHT HOOKED.h 2 /
//!COMPONENTS 2
//!DESC [quint] vector median filter on flow slot3->slot4 (pass 2)
vec4 hook() {
...
//!HEIGHT HOOKED.h 2 /
//!COMPONENTS 2
//!DESC [quint] vector median filter on flow slot4->slot3 (pass 2)
vec4 hook() {
    ivec2 coord = ivec2(FLOW_H_DE_pos * FLOW_H_DE_size);
    if (!pair_changed)
...
// =====================================================================
// SLOT 2 -> SLOT 3 flow chain ([quint], generated). The pair the fourth
...
// =====================================================================
// SLOT 3 -> SLOT 2 flow chain ([quint], generated). Reverse of the above,
...
//!HEIGHT HOOKED.h 16 /
//!COMPONENTS 4
//!DESC [quint] coarse flow search slot2->slot3 and slot3->slot2 (1/16 res) [fused: one dispatch] [foresight: fourth descent from the DE/ED 1/8 flow]
...
void coarse_ba() {
    ...
    vec2 fut_s = imageLoad(FLOW_E_ED_CACHE, coord * 2).xy * 0.5 * LUMA_A_S_pt;
    ...
    vec2 off_c = descend_s2(uv_b, prev_s, cost_c);
    float cost_f;
    vec2 off_f = descend_s2(uv_b, fut_s, cost_f);
    imageStore(FLOW_S_DC_CACHE2, coord, vec4(off_c / LUMA_A_S_pt, off_f / LUMA_A_S_pt));
    vec4 result = vec4(off_a / LUMA_A_S_pt, off_b / LUMA_A_S_pt);
    imageStore(FLOW_S_DC_CACHE, coord, result);
}
vec4 hook() {
    ...
    vec2 fut_s = imageLoad(FLOW_E_DE_CACHE, coord * 2).xy * 0.5 * LUMA_A_S_pt;
    ...
    vec2 off_c = descend_s(uv_a, prev_s, cost_c);
    float cost_f;
    vec2 off_f = descend_s(uv_a, fut_s, cost_f);
    imageStore(FLOW_S_CD_CACHE2, coord, vec4(off_c / LUMA_A_S_pt, off_f / LUMA_A_S_pt));
    vec4 result = vec4(off_a / LUMA_A_S_pt, off_b / LUMA_A_S_pt);
    imageStore(FLOW_S_CD_CACHE, coord, result);
    return result;
}
...
//!HEIGHT HOOKED.h 8 /
//!COMPONENTS 2
//!DESC [quint] refine flow slot2->slot3 (1/8 res) [foresight: fifth candidate and prior from the DE 1/8 flow]
...
const float SEED_TEMP_LAMBDA = 0.5;
const float SEED_RT_MAX = 1.0;   // E-texels; the previous flow must round-trip within this to be trusted
const float SEED_FUT_LAMBDA = 0.0;   // the foresight prior, toward the NEXT pair's flow: SEED_TEMP_LAMBDA mirrored in time
vec4 hook() {
    ivec2 coord = ivec2(LUMA_A_E_pos * LUMA_A_E_size);
    if (!pair_changed)
    ...
    bool trusted = rt < SEED_RT_MAX && length(prev_e) > 0.0;
    float tl = trusted ? SEED_TEMP_LAMBDA : 0.0;
    // ---- the foresight seed: the next pair's flow at this texel, trusted where IT round-trips ----
    vec2 fut_e = imageLoad(FLOW_E_DE_CACHE, coord).xy * LUMA_A_E_pt;
    ivec2 fcoord = clamp(ivec2((uv_a + fut_e) * LUMA_A_E_size), ivec2(0), ivec2(LUMA_A_E_size) - 1);
    vec2 fut_rev = imageLoad(FLOW_E_ED_CACHE, fcoord).xy * LUMA_A_E_pt;
    float rt_f = length((fut_e + fut_rev) / LUMA_A_E_pt);
    bool trusted_f = rt_f < SEED_RT_MAX && length(fut_e) > 0.0;
    float fl = trusted_f ? SEED_FUT_LAMBDA : 0.0;
    ...
    vec2 base_off4 = imageLoad(FLOW_S_CD_CACHE2, scoord).zw * 2.0 * LUMA_A_E_pt;
    float sad_a, sad_b, sad_c, sad_f;
    vec2 ref_a = refine_e(uv_a, base_off, sad_a);
    vec2 ref_b = refine_e(uv_a, base_off2, sad_b);
    vec2 ref_c = refine_e(uv_a, base_off3, sad_c);
    vec2 ref_f = refine_e(uv_a, base_off4, sad_f);
    // ZERO SEED (NFRAME-LIMITS.md section 9). The coarse level is point-sampled: on texture above its
    // own Nyquist it matches a Moire that is right only at integer coarse-texel shifts (the diagonal
    // speed ladder: (16,16) px/frame exact, (8,8) 61% locked to a texture-period copy, (4,4) 98%). This
    // level resolves that texture and reaches +/-2 of its texels from any seed, so a fourth seed at
    ...
    float score_a = sad_a + SEED_MAG_LAMBDA * length(ref_a / LUMA_A_E_pt) + tl * length((ref_a - prev_e) / LUMA_A_E_pt) + fl * length((ref_a - fut_e) / LUMA_A_E_pt);
    float score_b = sad_b + SEED_MAG_LAMBDA * length(ref_b / LUMA_A_E_pt) + tl * length((ref_b - prev_e) / LUMA_A_E_pt) + fl * length((ref_b - fut_e) / LUMA_A_E_pt);
    float score_c = trusted ? sad_c + SEED_MAG_LAMBDA * length(ref_c / LUMA_A_E_pt) + tl * length((ref_c - prev_e) / LUMA_A_E_pt) + fl * length((ref_c - fut_e) / LUMA_A_E_pt) : 1.0e30;
    ...
    float best_sad = (best_off == ref_a) ? sad_a : (best_off == ref_b) ? sad_b : (best_off == ref_c) ? sad_c : sad_f;
    if (d_ok) {
        if (moire > MOIRE_MIN || fdiff > DIFF_MIN) {
            float score_d = sad_d + SEED_MAG_LAMBDA * length(ref_d / LUMA_A_E_pt) + tl * length((ref_d - prev_e) / LUMA_A_E_pt) + fl * length((ref_d - fut_e) / LUMA_A_E_pt);
            if (score_d < best_score * (1.0 - TIE_MARGIN)) best_off = ref_d;
        } else if (sad_d < best_sad * (1.0 - ZERO_SEED_MARGIN)) {
            best_off = ref_d;
        }
...
//!HEIGHT HOOKED.h 8 /
//!COMPONENTS 2
//!DESC [quint] refine flow slot3->slot2 (1/8 res) [foresight: fifth candidate and prior from the ED 1/8 flow]
...
vec4 hook() {
    ...
    float tl = trusted ? SEED_TEMP_LAMBDA : 0.0;
    // ---- the foresight seed: the next pair's flow at this texel, trusted where IT round-trips ----
    vec2 fut_e = imageLoad(FLOW_E_ED_CACHE, coord).xy * LUMA_A_E_pt;
    ivec2 fcoord = clamp(ivec2((uv_b + fut_e) * LUMA_A_E_size), ivec2(0), ivec2(LUMA_A_E_size) - 1);
    vec2 fut_rev = imageLoad(FLOW_E_DE_CACHE, fcoord).xy * LUMA_A_E_pt;
    float rt_f = length((fut_e + fut_rev) / LUMA_A_E_pt);
    bool trusted_f = rt_f < SEED_RT_MAX && length(fut_e) > 0.0;
    float fl = trusted_f ? SEED_FUT_LAMBDA : 0.0;
    ivec2 scoord = ivec2(snap_texel(uv_b, LUMA_B_S_size) * LUMA_B_S_size);
    vec2 base_off3 = imageLoad(FLOW_S_DC_CACHE2, scoord).xy * 2.0 * LUMA_A_E_pt;
    vec2 base_off4 = imageLoad(FLOW_S_DC_CACHE2, scoord).zw * 2.0 * LUMA_A_E_pt;
    float sad_a, sad_b, sad_c, sad_f;
    vec2 ref_a = refine_e(uv_b, base_off, sad_a);
    vec2 ref_b = refine_e(uv_b, base_off2, sad_b);
    vec2 ref_c = refine_e(uv_b, base_off3, sad_c);
    vec2 ref_f = refine_e(uv_b, base_off4, sad_f);
    // ZERO SEED (NFRAME-LIMITS.md section 9). The coarse level is point-sampled: on texture above its
    // own Nyquist it matches a Moire that is right only at integer coarse-texel shifts (the diagonal
    // speed ladder: (16,16) px/frame exact, (8,8) 61% locked to a texture-period copy, (4,4) 98%). This
    // level resolves that texture and reaches +/-2 of its texels from any seed, so a fourth seed at
...
//!HEIGHT HOOKED.h 4 /
//!COMPONENTS 2
//!DESC [quint] refine flow slot2->slot3 (1/4 res)
...
//!HEIGHT HOOKED.h 4 /
//!COMPONENTS 2
//!DESC [quint] refine flow slot3->slot2 (1/4 res)
...
//!HEIGHT HOOKED.h 2 /
//!COMPONENTS 2
//!DESC [quint] refine flow slot2->slot3 (half res)
...
//!HEIGHT HOOKED.h 2 /
//!COMPONENTS 2
//!DESC [quint] refine flow slot3->slot2 (half res)
...
//!HEIGHT HOOKED.h 2 /
//!COMPONENTS 2
//!DESC [quint] vector median filter on flow slot2->slot3 (pass 1) [fused with its B->A twin: one dispatch]
void hook_ba() {
    if (!pair_changed)
...
//!HEIGHT HOOKED.h 2 /
//!COMPONENTS 2
//!DESC [quint] vector median filter on flow slot2->slot3 (pass 2)
vec4 hook() {
...
//!HEIGHT HOOKED.h 2 /
//!COMPONENTS 2
//!DESC [quint] vector median filter on flow slot3->slot2 (pass 2)
vec4 hook() {
    ivec2 coord = ivec2(FLOW_H_CD_pos * FLOW_H_CD_size);
    if (!pair_changed)
...
// =====================================================================
// SLOT 1 -> SLOT 2 flow chain ([quint], generated). The base's slot-0 ->
...
// =====================================================================
// SLOT 2 -> SLOT 1 flow chain ([quint], generated). Reverse of the above,
...
//!HEIGHT HOOKED.h 16 /
//!COMPONENTS 4
//!DESC [quint] coarse flow search slot1->slot2 and slot2->slot1 (1/16 res) [fused: one dispatch] [foresight: fourth descent from the CD/DC 1/8 flow]
...
void coarse_ba() {
    ...
    vec2 fut_s = imageLoad(FLOW_E_DC_CACHE, coord * 2).xy * 0.5 * LUMA_A_S_pt;
    ...
    float cost_f;
    vec2 off_f = descend_s2(uv_b, fut_s, cost_f);
    imageStore(FLOW_S_CB_CACHE2, coord, vec4(off_c / LUMA_A_S_pt, off_f / LUMA_A_S_pt));
    vec4 result = vec4(off_a / LUMA_A_S_pt, off_b / LUMA_A_S_pt);
    imageStore(FLOW_S_CB_CACHE, coord, result);
}
vec4 hook() {
    ...
    vec2 fut_s = imageLoad(FLOW_E_CD_CACHE, coord * 2).xy * 0.5 * LUMA_A_S_pt;
    ...
    float cost_f;
    vec2 off_f = descend_s(uv_a, fut_s, cost_f);
    imageStore(FLOW_S_BC_CACHE2, coord, vec4(off_c / LUMA_A_S_pt, off_f / LUMA_A_S_pt));
    vec4 result = vec4(off_a / LUMA_A_S_pt, off_b / LUMA_A_S_pt);
    imageStore(FLOW_S_BC_CACHE, coord, result);
    return result;
}
...
//!HEIGHT HOOKED.h 8 /
//!COMPONENTS 2
//!DESC [quint] refine flow slot1->slot2 (1/8 res) [foresight: fifth candidate and prior from the CD 1/8 flow]
...
vec4 hook() {
    ...
    float tl = trusted ? SEED_TEMP_LAMBDA : 0.0;
    // ---- the foresight seed: the next pair's flow at this texel, trusted where IT round-trips ----
    vec2 fut_e = imageLoad(FLOW_E_CD_CACHE, coord).xy * LUMA_A_E_pt;
    ivec2 fcoord = clamp(ivec2((uv_a + fut_e) * LUMA_A_E_size), ivec2(0), ivec2(LUMA_A_E_size) - 1);
    vec2 fut_rev = imageLoad(FLOW_E_DC_CACHE, fcoord).xy * LUMA_A_E_pt;
    float rt_f = length((fut_e + fut_rev) / LUMA_A_E_pt);
    bool trusted_f = rt_f < SEED_RT_MAX && length(fut_e) > 0.0;
    float fl = trusted_f ? SEED_FUT_LAMBDA : 0.0;
    ...
    vec2 base_off4 = imageLoad(FLOW_S_BC_CACHE2, scoord).zw * 2.0 * LUMA_A_E_pt;
    float sad_a, sad_b, sad_c, sad_f;
    vec2 ref_a = refine_e(uv_a, base_off, sad_a);
    vec2 ref_b = refine_e(uv_a, base_off2, sad_b);
    vec2 ref_c = refine_e(uv_a, base_off3, sad_c);
...
//!HEIGHT HOOKED.h 8 /
//!COMPONENTS 2
//!DESC [quint] refine flow slot2->slot1 (1/8 res) [foresight: fifth candidate and prior from the DC 1/8 flow]
...
vec4 hook() {
    ...
    float tl = trusted ? SEED_TEMP_LAMBDA : 0.0;
    // ---- the foresight seed: the next pair's flow at this texel, trusted where IT round-trips ----
    vec2 fut_e = imageLoad(FLOW_E_DC_CACHE, coord).xy * LUMA_A_E_pt;
    ivec2 fcoord = clamp(ivec2((uv_b + fut_e) * LUMA_A_E_size), ivec2(0), ivec2(LUMA_A_E_size) - 1);
    vec2 fut_rev = imageLoad(FLOW_E_CD_CACHE, fcoord).xy * LUMA_A_E_pt;
    float rt_f = length((fut_e + fut_rev) / LUMA_A_E_pt);
    bool trusted_f = rt_f < SEED_RT_MAX && length(fut_e) > 0.0;
    float fl = trusted_f ? SEED_FUT_LAMBDA : 0.0;
    ivec2 scoord = ivec2(snap_texel(uv_b, LUMA_B_S_size) * LUMA_B_S_size);
    vec2 base_off3 = imageLoad(FLOW_S_CB_CACHE2, scoord).xy * 2.0 * LUMA_A_E_pt;
    vec2 base_off4 = imageLoad(FLOW_S_CB_CACHE2, scoord).zw * 2.0 * LUMA_A_E_pt;
    float sad_a, sad_b, sad_c, sad_f;
    vec2 ref_a = refine_e(uv_b, base_off, sad_a);
    vec2 ref_b = refine_e(uv_b, base_off2, sad_b);
    vec2 ref_c = refine_e(uv_b, base_off3, sad_c);
...
//!HEIGHT HOOKED.h 4 /
//!COMPONENTS 2
//!DESC [quint] refine flow slot1->slot2 (1/4 res)
...
//!HEIGHT HOOKED.h 4 /
//!COMPONENTS 2
//!DESC [quint] refine flow slot2->slot1 (1/4 res)
...
//!HEIGHT HOOKED.h 2 /
//!COMPONENTS 2
//!DESC [quint] refine flow slot1->slot2 (half res)
...
//!HEIGHT HOOKED.h 2 /
//!COMPONENTS 2
//!DESC [quint] refine flow slot2->slot1 (half res)
...
//!HEIGHT HOOKED.h 2 /
//!COMPONENTS 2
//!DESC [quint] vector median filter on flow slot1->slot2 (pass 1) [fused with its B->A twin: one dispatch]
void hook_ba() {
    if (!pair_changed)
...
//!HEIGHT HOOKED.h 2 /
//!COMPONENTS 2
//!DESC [quint] vector median filter on flow slot1->slot2 (pass 2)
vec4 hook() {
...
//!HEIGHT HOOKED.h 2 /
//!COMPONENTS 2
//!DESC [quint] vector median filter on flow slot2->slot1 (pass 2)
vec4 hook() {
    ivec2 coord = ivec2(FLOW_H_BC_pos * FLOW_H_BC_size);
    if (!pair_changed)
...
//!HEIGHT HOOKED.h
//!COMPONENTS 1
//!DESC [quint] slot 0 luma at full res
vec4 hook() {
    return vec4(dot(HOOKED_tex(HOOKED_pos).rgb, vec3(0.299, 0.587, 0.114)), 0, 0, 0);
}
//!HOOK FRAME_MIX
...
//!HEIGHT HOOKED.h
//!COMPONENTS 1
//!DESC [quint] slot 1 luma at full res
vec4 hook() {
    return vec4(dot(FRAME1_tex(HOOKED_pos).rgb, vec3(0.299, 0.587, 0.114)), 0, 0, 0);
}
//!HOOK FRAME_MIX
...
//!HEIGHT HOOKED.h
//!COMPONENTS 1
//!DESC [quint] slot 2 luma at full res
vec4 hook() {
    return vec4(dot(FRAME2_tex(HOOKED_pos).rgb, vec3(0.299, 0.587, 0.114)), 0, 0, 0);
}
//!HOOK FRAME_MIX
...
//!HEIGHT HOOKED.h
//!COMPONENTS 1
//!DESC [quint] slot 3 luma at full res
vec4 hook() {
    return vec4(dot(FRAME3_tex(HOOKED_pos).rgb, vec3(0.299, 0.587, 0.114)), 0, 0, 0);
}
//!HOOK FRAME_MIX
...
//!HEIGHT HOOKED.h
//!COMPONENTS 1
//!DESC [quint] slot 4 luma at full res
vec4 hook() {
    return vec4(dot(FRAME4_tex(HOOKED_pos).rgb, vec3(0.299, 0.587, 0.114)), 0, 0, 0);
}
...
//!HEIGHT HOOKED.h
//!COMPONENTS 2
//!DESC [quint] refine flow A->B (full res)
...
//!HEIGHT HOOKED.h
//!COMPONENTS 2
//!DESC [quint] refine flow B->A (full res)
...
//!HEIGHT HOOKED.h
//!COMPONENTS 2
//!DESC [quint] refine flow slot1->slot2 (full res)
...
//!HEIGHT HOOKED.h
//!COMPONENTS 2
//!DESC [quint] refine flow slot2->slot1 (full res)
...
//!HEIGHT HOOKED.h
//!COMPONENTS 2
//!DESC [quint] refine flow slot2->slot3 (full res)
...
//!HEIGHT HOOKED.h
//!COMPONENTS 2
//!DESC [quint] refine flow slot3->slot2 (full res)
...
//!HEIGHT HOOKED.h
//!COMPONENTS 2
//!DESC [quint] refine flow slot3->slot4 (full res)
...
//!HEIGHT HOOKED.h
//!COMPONENTS 2
//!DESC [quint] refine flow slot4->slot3 (full res)
...
//!WIDTH HOOKED.w
//!HEIGHT HOOKED.h
//!DESC [quint] pack the full-res flows slot0->slot1 (rg) and slot1->slot0 (ba)
vec4 hook() {
...
//!WIDTH HOOKED.w
//!HEIGHT HOOKED.h
//!DESC [quint] pack the full-res flows slot1->slot2 (rg) and slot2->slot1 (ba)
vec4 hook() {
...
//!WIDTH HOOKED.w
//!HEIGHT HOOKED.h
//!DESC [quint] pack the full-res flows slot2->slot3 (rg) and slot3->slot2 (ba)
vec4 hook() {
