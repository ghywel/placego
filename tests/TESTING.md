...
Every render time in this repository before this date was taken with the
scene generated by `lavfi` inside the same ffmpeg run. That source is a
per-pixel expression evaluated on the CPU and costs about 38 ms per 720p
frame -- seventy percent of a two-frame shader's total and forty percent of
the four-frame's. Differences between shaders measured that way are real;
the percentages are diluted. Time from a pre-rendered file:

    ffmpeg -f lavfi -i "$(source tests/scenes.sh; scene O5_osc_textured 24)" -frames:v 61 -c:v ffv1 -level 3 o5.mkv
    ffmpeg -benchmark -init_hw_device vulkan=vk -filter_hw_device vk -i o5.mkv \
           -vf "libplacebo=fps=60:frame_mixer=custom_n:custom_shader_path=shader.glsl" -f null -

interleaved with the shader it is compared against, after a warm-up run,
median of at least three pairs. On the RX 6600 that gives the stock mixer
0.74 s for sixty frames, the two-frame propagated shader 1.42 s, the quad
