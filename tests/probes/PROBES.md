...
| `dig/` | dig.sh | the defect funnel on real material: prospect a window, cut each candidate frame-exactly, decimate-and-reconstruct it with hold/linear/the shader, rank by the shader's margin over linear (the prospector finds estimator disagreement, not visible faults) | Content drawn on twos (2026-09-19), the funnel paragraph | `np-scratch/dig/<label>/` |
