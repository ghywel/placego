...
| `accelcheck.py` | Calibrates the acceleration field against analytically known truth. Reports COVERAGE and conditional accuracy, because the field is legitimately sparse and a single average over the gaps is a category error. `FIELD=jerk` switches the analytic truth to -A*w^3*cos(wt)/fps^3 for the quad shader's TRI_DIAG=5 jerk field (same readback, one env var). |
