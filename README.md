...
- **How it was done.** [METHODOLOGY.md](METHODOLOGY.md) on the working
  ...
  the way.
- **A native port.** The Metal app, published by the owner as a signed and notarized
  `NFrameDemo-macos-arm64-v<N>.zip` (its source is not published), is the four-frame shader
  machine-translated to Metal and wrapped in a small macOS app, with
  [METALPORT.md](METALPORT.md) as its record.
